---
name: kapable:deflake-e2e-from-run
...
2. List artifacts and find the `html-report` (merged across shards):
   ```
   gh api repos/AkshatSidharth/KapAble/actions/runs/<run_id>/artifacts --jq '.artifacts[] | {name, size_in_bytes}'
   ```
3. Download it into a scratch dir (use `-R AkshatSidharth/KapAble` — `gh run download` does not auto-detect the repo from arbitrary cwd):
   ```
   mkdir -p /tmp/pw-report
   gh run download <run_id> -R AkshatSidharth/KapAble -n html-report -D /tmp/pw-report
   ```
...
5. As the final step, call `/kapable:pr-push`. Do not commit, push, or create the PR directly from this skill. When invoking `/kapable:pr-push`, carry forward enough context for the PR body to include:
