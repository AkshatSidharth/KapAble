---
name: kapable:pr-fix:ci
description: Diagnose and fix failing CI checks on a pull request from its latest GitHub Actions CI run. Use when PR CI has E2E/Playwright failures, macOS or Windows unit-test failures, or presubmit/type/build failures; analyzes the exact run logs and artifacts, invokes kapable:deflake-e2e-from-run for E2E failures, reproduces targeted unit tests locally, and publishes the fixes.
---
...
3. Find the newest `CI` workflow run for the PR's current head SHA:
   ...
   ```sh
   gh run list -R AkshatSidharth/KapAble --workflow CI --commit <HEAD_SHA> \
...
4. Enumerate every job and its failed step:
   ...
   ```sh
   gh api repos/AkshatSidharth/KapAble/actions/runs/<RUN_ID>/jobs --paginate \
...
1. Download each job log separately:
   ...
   ```sh
   gh run view <RUN_ID> -R AkshatSidharth/KapAble --job <JOB_ID> --log > <SCRATCH_DIR>/<OS>.log
   ```
...
After unit-test fixes are ready, list the run artifacts. If an E2E shard failed and `html-report` exists, invoke `/kapable:deflake-e2e-from-run` with the selected run URL. Follow its artifact-first trace analysis, rebuild, and targeted E2E verification. Running it after unit fixes lets its required `/kapable:pr-push` publish the complete CI fix together.

If `merge-reports` failed, inspect that job's failed step before delegating. An install, artifact-download, or merge failure can occur before `html-report` is uploaded. Diagnose reporting-infrastructure failures from the merge job log. When shard failures also need trace analysis but `html-report` is absent, list and download every retained `blob-report-*` artifact into one scratch directory, then run Playwright's merge command locally to reconstruct the HTML report before following `/kapable:deflake-e2e-from-run`'s report-analysis phases:

```sh
gh api repos/AkshatSidharth/KapAble/actions/runs/<RUN_ID>/artifacts --paginate \
  --jq '.artifacts[] | select(.name | startswith("blob-report-")) | .name' \
  > <SCRATCH_DIR>/blob-artifact-names
while IFS= read -r artifact; do
  gh run download <RUN_ID> -R AkshatSidharth/KapAble -n "$artifact" \
...
If `/kapable:deflake-e2e-from-run` did not already publish the combined changes, invoke `/kapable:pr-push`. Report the source run URL, each failed job's concrete root cause and fix, commands run locally, and any platform result that still requires CI confirmation.
