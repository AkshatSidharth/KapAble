...
**Title:** `[session report] No description (KapAble <version>, <Windows|macOS|Linux>)`
...
**Tell the reporter:** KapAble can't find Node.js on the computer, which it needs to
run the app. Steps:
...
3. If it still doesn't work, open **Settings**, find **Node.js Path
   Configuration**, and click **Browse for Node.js** to point KapAble at the folder
...
**Assessment:** `likely_kapable_bug` if on a version older than 1.13.0, otherwise
`needs_info`.
...
**When you see:** KapAble Pro credits are used up, a daily chat limit message, or
the reporter wants KapAble to use their own API key instead of Pro.

**Assessment:** `question`

**Tell the reporter:** click the **Pro** button next to the model picker and turn
off **Enable KapAble Pro**. KapAble will use their own API key until they turn it
back on.
...
**Tell the reporter:** someone from the KapAble team will check the account. Ask
...
exists in `kapable-apps` (often after a crash mid-import).

**Assessment:** `fixed_in_release` (1.10.0-beta.3)

**Tell the reporter:** delete or rename that folder inside `kapable-apps`, then
...
requests failing while the KapAble servers report no errors.
...
Fable declines some of them. Since 1.8.0-beta.1 KapAble shows a warning when this
happens.
...
**When you see:** KapAble quits on its own, freezes, or force closes.
...
log file from `%APPDATA%\kapable\logs` on Windows.
...
**Tell the reporter:** KapAble needs a recent version of macOS; upgrade macOS and
try again.
...
loop, a runtime error in their code, their page failing), not in KapAble itself.

**Assessment:** `user_app_issue`

**Tell the reporter:** this is coming from the app rather than from KapAble. Switch
to Agent mode and ask KapAble to find the cause.
...
**When you see:** the reporter's KapAble version is behind the current one.

**Assessment:** keep whatever else applies; add the step below.

**Tell the reporter:** download the latest version from
https://www.kapable.sh/download. If a release fixed this exact problem, say which
...
**Tell the reporter:** that balance is managed by the provider, not by KapAble;
