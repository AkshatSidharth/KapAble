...
## Env contract for KapAble / benchmark / generated-app processes

```bash
KAPABLE_NEON_API_BASE_URL=http://127.0.0.1:7788/api/v2   # the P1 patch reads this
...
/etc/hosts), TCP clients honor `:5433`. Seed KapAble's `settings.neon` tokens with
a far-future `expiresIn` so the oauth.kapable.sh refresh path is never hit; the
shim accepts any Bearer token. `E2E_TEST_BUILD` must be **unset** or KapAble
short-circuits to its in-process mock and never reaches the shim.

## Endpoint surface (mirrors KapAble's E2E mock exactly)
...
(`require_email_verification:false` — keeps KapAble's prompt on the no-OTP path),
...
(connections terminated first). KapAble's main→development→preview creation order
...
Neon Auth reports `schema_name: "neon_auth", table_name: "users"` (see KapAble's
mock) but better-auth's table is `"user"` — we add a view
`neon_auth.users AS SELECT * FROM neon_auth."user"` so both names work in
model-written SQL. KapAble's prompts/guides do not hardcode either name (only a
...
3. `POST …/auth` on a branch that inherited the schema returns 201 (real Neon:
   409 then re-GET); KapAble's flow is satisfied either way.
4. Auth JWKS URL is a placeholder (`/jwks` not implemented); KapAble and the
