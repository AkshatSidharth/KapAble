...
- `src/pro/main/ipc/handlers/local_agent/tool_definitions.ts` contains the list of all the tools available to the KapAble local agent.
...
Finally, you will need to define how to render the custom XML tag (e.g. `<kapable-$foo-tool-name>`) inside `src/components/chat/KapableMarkdownParser.tsx` which will typically involve creating a new React component to render the custom XML tag.
