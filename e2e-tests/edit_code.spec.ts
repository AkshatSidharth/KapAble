...
test("edit code", async ({ po }) => {
  ...
  await replaceEditorContent(po.page, "export const MadeWithKapable = ;");
  ...
  await expectFileContent(
    appPath,
    editedFilePath,
    "export const MadeWithKapable = ;",
  );
  const editedFile = fs.readFileSync(
    path.join(appPath, editedFilePath),
    "utf8",
  );
  expect(editedFile).toContain("export const MadeWithKapable = ;");
});
...
test("edit code edits the right file during rapid switches", async ({ po }) => {
  ...
  for (const round of [1, 2, 3]) {
    firstFileEdit = `export const MadeWithKapable = "round-${round}";\n`;
