...
export async function launchElectronApp({
  ...
  process.env.KAPABLE_E2E_PORT_BLOCK_INDEX = String(parallelIndex);
  ...
  process.env.KAPABLE_ENGINE_URL = `http://localhost:${fakeLlmPort}/engine/v1`;
  process.env.KAPABLE_GATEWAY_URL = `http://localhost:${fakeLlmPort}/gateway/v1`;
  ...
  (electronApp as any).$kapableUserDataDir = userDataDir;
  ...
  po: [
    async ({ electronApp, electronConfig }, use, testInfo) => {
      ...
      const po = new PageObject(electronApp, page, {
        userDataDir: (electronApp as any).$kapableUserDataDir,
  ...
  electronApp: [
    async ({ electronConfig }, use, testInfo) => {
      ...
      const userDataDir = path.join(
        baseTmpDir,
        `kapable-e2e-tests-worker-${testInfo.parallelIndex}-${Date.now()}`,
      );
      ...
      if (os.platform() === "win32") {
        try {
          ...
          console.warn(
            "Failed to kill kapable.exe: (continuing with test cleanup)",
            error,
          );
        }
      } else {
