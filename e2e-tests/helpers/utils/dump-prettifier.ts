    ...
    .map((message) => {
      const content = Array.isArray(message.content)
        ? JSON.stringify(message.content)
            // Normalize attachment paths (dynamic MD5 hashes in .kapable/media)
            .replace(
              /path: [^"]*?[/\\]{1,2}\.kapable[/\\]{1,2}media[/\\]{1,2}[a-f0-9]+\.\w+/g,
              "path: [[ATTACHMENT_PATH]]",
            )
            // Also normalize .kapable/media paths in escaped attribute format
            .replace(
              /[/\\]{1,2}\.kapable[/\\]{1,2}media[/\\]{1,2}[a-f0-9]{6,}\.\w+/g,
              "/.kapable/media/[[ATTACHMENT_HASH]]",
            )
        : message.content
            ...
            .replace(
              /\n<kapable-file path="package\.json">[\s\S]*?<\/kapable-file>\n/g,
              "",
            );
