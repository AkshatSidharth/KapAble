 ...
 *   instead of "kapable Safe Storage", and that (wrong) key is cached for the
 *   rest of the process.
 * - On Electron 43, safeStorage refuses to run pre-ready and post-ready always
 *   uses the proper "kapable Safe Storage" identity — so ciphertext produced under
 ...
 *   the `security` CLI ("kapable Safe Storage"/"kapable" and
 ...
 *   resolved the other. Kill switch: KAPABLE_DISABLE_SAFE_STORAGE_RECOVERY=1.
 *
 ...
 * with "Chromium Safe Storage"/"kapable Safe Storage" entries shared with real
 ...
 *
 *   npm run pre:e2e
 *   KAPABLE_E2E_SAFE_STORAGE=1 npx playwright test \
 ...
 *
 *   KAPABLE_E2E_SAFE_STORAGE=1 \
 *   KAPABLE_E2E_SAFE_STORAGE_UPGRADE_BUILD=/path/to/e43/out/kapable-darwin-arm64 \
...
const ENABLED =
  process.platform === "darwin" && process.env.KAPABLE_E2E_SAFE_STORAGE === "1";

const UPGRADE_BUILD_DIR = process.env.KAPABLE_E2E_SAFE_STORAGE_UPGRADE_BUILD;
...
const KAPABLE_SERVICE = "kapable Safe Storage";
const CHROMIUM_SERVICE = "Chromium Safe Storage";

const KEYCHAIN_PASSWORD = "kapable-e2e-safe-storage";
const TEMP_KEYCHAIN = path.join(
  os.tmpdir(),
  `kapable-e2e-safe-storage-${process.pid}.keychain-db`,
);
...
function freshTempKeychain(): void {
  ...
  for (const [service, account, password] of [
    ...
    [KAPABLE_SERVICE, "kapable Key", "e2e-kapable-identity-key"],
  ]) {
    security([
...
async function launchKapable({
  ...
  try {
    ...
    await closeKapable(electronApp);
    throw error;
  }
}

async function closeKapable(electronApp: ElectronApplication): Promise<void> {
...
function makeUserDataDir(label: string): string {
  const dir = path.join(
    os.tmpdir(),
    `kapable-e2e-safe-storage-${label}-${Date.now()}`,
  );
...
test.describe("safeStorage keychain identity (issue #3837)", () => {
  test.skip(
    !ENABLED,
    "Opt-in: requires macOS and KAPABLE_E2E_SAFE_STORAGE=1 (temporarily swaps the default keychain)",
  );
  ...
  test.afterEach(async () => {
    await Promise.all(
      [...launchedApps].map((electronApp) => closeKapable(electronApp)),
    );
  });
  ...
  test("stored secret survives the pre-ready identity flip across restarts (Layer 1 recovery)", async () => {
    ...
    // the Keychain entry under the proper "kapable Safe Storage" identity.
    const session1 = await launchKapable({ userDataDir });
    const ciphertext = await encryptViaApp(session1, token);
    expect(await decryptViaApp(session1, ciphertext)).toBe(token);
    await closeKapable(session1);
    ...
    // Safe Storage" identity, which cannot decrypt the kapable-identity ciphertext
    // from session 1. This race is deliberately left unfixed; the fix instead
    // makes the secret survive it.
    freshTempKeychain();
    const session2 = await launchKapable({ userDataDir });
  ...
  test("Electron 40 -> 43 upgrade: pre-fix build still drops steady-state secrets (#3837)", async () => {
    test.skip(
      !UPGRADE_BUILD_DIR,
      "Set KAPABLE_E2E_SAFE_STORAGE_UPGRADE_BUILD to a packaged build dir (e.g. out/kapable-darwin-arm64) built at the Electron 43 commit (parent of revert d24360e8)",
    );
    test.setTimeout(240_000);
    const userDataDir = makeUserDataDir("upgrade");
    const token = "gh_e2e_upgrade_secret";

    // NOTE: the upgrade build (KAPABLE_E2E_SAFE_STORAGE_UPGRADE_BUILD) is built at
    ...
    // use and post-ready always uses the proper "kapable Safe Storage" identity, so
    ...
    // Layer 1 recovery.
    freshTempKeychain();
    const session3 = await launchKapable({
      userDataDir,
