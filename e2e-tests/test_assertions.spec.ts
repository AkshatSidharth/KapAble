...
testSkipIfWindows(
  ...
  async ({ po }) => {
    ...
    const card = po.page.getByTestId("kapable-test-assertions-card");
    ...
    await expect(
      card.locator('[data-testid^="kapable-test-assertions-step-"]').first(),
    ).toBeVisible();

    const assertions = card.locator(
      '[data-testid^="kapable-test-assertions-assertion-"]',
    );
    ...
    await expect(card).toContainText("KapAble is waiting on this before it");
    await expect(
      po.page.getByTestId("kapable-test-assertions-discard-button"),
    ).toBeVisible();
    ...
    await po.page.getByTestId("kapable-test-assertions-approve-button").click();
    await expect(
      po.page.getByTestId("kapable-test-assertions-approved-badge"),
...
testSkipIfWindows(
  ...
  async ({ po }) => {
    ...
    await expect(
      po.page.getByTestId("kapable-test-assertions-discarded-note"),
    ).toBeVisible({ timeout: Timeout.LONG });
