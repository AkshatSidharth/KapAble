...
- The real `chat:stream` handler + tag processor + git + sqlite run under plain-node
  ...
  `kapable_tags_parsing` equivalent runs in **~1.3–2s vs 30–90s** in Playwright.
...
- `src/testing/chat_flow_harness.ts` — `setupChatFlowHarness()` returning
  ...
  via `KAPABLE_DEV_USER_DATA_DIR`, app's own `initializeDatabase()`, fixture-app + git
...
- Refactor `testing/fake-llm-server` to export `createApp(fixturesDir)` without
  ...
  First migrations (each deletes or demotes an e2e spec): `kapable_tags_parsing`,
