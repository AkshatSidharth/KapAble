...
- Attribute individual dirty files to the agent with certainty; KapAble already cannot distinguish partial generation writes from simultaneous manual edits.
...
On success, return authoritative `appId`, branch, accepted HEAD, and optional saved-version ID; transition to `closed`; remove the checkpoint through the existing persistence observer; reset historical-preview presentation; refresh repository, version, file, and chat data; dismiss the recovery toast; and restore normal Version History capabilities. On failure, remain in `restore-recovery-required` and replace the description with an actionable `KapableErrorKind.Conflict` or `Precondition` message.
...
2. stage the same user-visible files covered by KapAble's existing whole-tree checkpoint behavior;
...
If staging, committing, or final validation fails, leave `restore-recovery-required` in place and show the specific error. Never reset, discard, force-checkout, or clear recovery on a partial failure. If KapAble crashes after the commit but before acknowledgement, restart reconciliation sees a clean repository and the user can safely choose **Use current version**.

Detached HEAD, unresolved index entries, missing repositories, and active merge/rebase/cherry-pick/revert/bisect operations do not offer the save action. KapAble does not attempt to repair, continue, abort, reset, or back up these states in this feature. The toast explains the detected blocker in plain language and offers only the read-only **Check again** action after the user resolves it outside KapAble.
...
4. KapAble recognizes the target turn as cancelled.
5. If the tree is dirty, KapAble creates `[Interrupted] Saved partial changes before restoring to an earlier version` and then completes the requested Undo or restore/fork flow.
...
> KapAble could not verify an earlier restore. If this project is clean, you can continue from its current version.  
> **Use current version**
...
> KapAble found changes that are not part of a saved version. Save them as the current version to continue using Version History.  
...
> **Version History is unavailable**  
> A Git rebase is still in progress. Finish or cancel it outside KapAble, then check again.  
> **Check again**
...
5. Preserve `KapableError` classification and original user-facing messages. A dirty tree remains `KapableErrorKind.Conflict` and should be filtered from exception telemetry.
...
- Completed/unknown target turn + dirty tree returns `KapableErrorKind.Conflict`, leaves branch/HEAD/index/tree unchanged, and does not create a fork chat.
...
- Stop a generation after a file write, then click Undo: KapAble saves partial work and successfully restores/forks without a dirty-tree error.
...
- A user whose only blocker is a dirty tree can choose **Save changes & use current version** without opening a terminal; KapAble creates a recoverable commit before clearing recovery.
...
Never log prompts, file contents, tokens, or remote URLs. Expected dirty-tree and repository-health refusals remain classified `KapableError`s rather than exception telemetry.
