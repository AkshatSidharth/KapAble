...
- **Four missed preview-error writers** in `PreviewIframe.tsx` (`:415-426`
  cloud-sandbox errors with source `kapable-app`, `:438-445` cloud sync errors,
  `:449-451` sync-recovery clear, `:1501` dismiss-any). `kapable-app` errors
...
{type:"idle"})` (hook returns `StreamState | undefined` — every
  ...
  - `PromoMessage.tsx:155`, `KapableOutput.tsx:28`,
    `KapableMarkdownParser.tsx:132` → same hook + fallback.
  ...
  - Tests: `manager.test.ts:175/183/210/215`,
    ...
    (fake chatStream facade), `KapableMarkdownParser.test.tsx:164`,
...
- `KapableMarkdownParser.tsx:248` → `useChatStreamPreview(chatId)`;
...
- **No machine owns the full state**: three sources across six writer
  sites. app_run `commands.ts:57-62` (`kapable-app`); preview_iframe
  `commands.ts:45` clear (cross-machine, runs in beforeNotify —
  synchronous facade calls forbidden); `useRunApp.ts:175-186/:191-195`
  (`kapable-sync`, priority-merge updaters); PreviewIframe `setErrorMessage`
  (`:223-231`) with call sites `:884/:901` (`preview-app`) **plus the four
  the trace missed**: `:415-426` cloud-sandbox errors with source
  `kapable-app` (so app_run state alone can never replace the reader),
  `:438-445` cloud sync errors (`kapable-sync`, same clobber guard),
...
- Owner decision: **preview_iframe** — it already issues both clears and
  ...
  command pipeline).
- Encode in transitions: source-priority updater semantics (kapable-sync must
  not clobber preview-app/kapable-app; only-clear-own-source on recovery),
...
- Reader `PreviewIframe.tsx:221` → `selectPreviewError(iframeState)` (it
  ...
  is load-bearing (`:1515` hasStartupError only for `kapable-app`).
...
`useStreamChat.test.tsx`, `KapableMarkdownParser.test.tsx`,
`explore_chat_history_streaming.integration.test.tsx`,
