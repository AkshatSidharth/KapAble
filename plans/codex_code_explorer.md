...
- If the app context is only resolved at execution time, fail with `KapableErrorKind.Precondition` and a concise message such as `Code explorer requires TypeScript to be installed and configured in this app.`

This differs from the existing TS checker fallback behavior: `explore_code` should not use bundled KapAble TypeScript to explore arbitrary projects. The experiment is meant to measure TypeScript-aware navigation on projects that are actually configured for TypeScript.
...
The worker should load TypeScript from the target app using Node resolution from the app root. If that fails, report the precondition error instead of falling back to KapAble's bundled TypeScript.
...
- Keep `typescript` external so resolution comes from the target app, not from KapAble.
...
```xml
<kapable-explore-code>...</kapable-explore-code>
```
...
- Add a stable test id such as `kapable-explore-code`.

## Error Handling

Use `KapableError` for expected user/project failures:

- `KapableErrorKind.Precondition`: missing app-local TypeScript, missing tsconfig, no source files, unsupported project shape.
- `KapableErrorKind.Validation`: invalid arguments, invalid bounds, escaping `tsconfig_path`.
- `KapableErrorKind.NotFound`: unknown `app_name`, via existing app resolution.
...
- Expected readiness failures are classified as `KapableErrorKind.Precondition`.
- Chat parser and renderer recognize `kapable-explore-code`.
...
- New E2E spec should:
  ...
  - assert the rendered `kapable-explore-code` card is visible
...
The benchmark should launch packaged KapAble programmatically with a real LLM provider and run paired baseline/experiment trials.
...
- Same packaged KapAble build.
- Same KapAble Engine provider path.
- Read `KAPABLE_PRO_KEY` from `.env` and use it to authenticate KapAble Engine calls.
...
- Local-agent mode.
- Real KapAble Engine calls only; no fake-provider benchmark path.
...
- Add benchmark-only JSONL recording behind an environment variable such as `KAPABLE_BENCHMARK_RUN_ID`.
...
- Load `.env`, require `KAPABLE_PRO_KEY`, and configure the app to use KapAble Engine for both arms.
...
- Do not replace KapAble's real agent loop with a standalone AI SDK harness; the benchmark should measure the product path users actually exercise.
...
- Document benchmark prerequisites clearly, including `KAPABLE_PRO_KEY` in `.env`, packaged build requirement, and whether repo dependencies should be installed before running.

## Risks And Mitigations

- Target app lacks installed TypeScript: hide the tool when readiness can be checked before request construction; otherwise return `KapableErrorKind.Precondition`.
...
- Benchmark contaminates normal app behavior: write JSONL only when `KAPABLE_BENCHMARK_RUN_ID` is set.
...
7. Add chat streaming parser and renderer support for `kapable-explore-code`.
8. Add the E2E fixture, fake local-agent fixture, and targeted E2E spec.
9. Add benchmark instrumentation behind `KAPABLE_BENCHMARK_RUN_ID`.
10. Add the packaged-KapAble benchmark CLI, repo/task manifests, result writer, and summary report.
