...
KapAble currently has several related but distinct state-machine architectures:
...
`KapableError`/`KapableErrorKind` where the existing IPC error path is more
...
and renderer ALWAYS ship together in production — kapable updates via
...
- Multiple main processes or networked KapAble instances.
...
- Building Erlang/OTP, Akka, or XState inside KapAble.
