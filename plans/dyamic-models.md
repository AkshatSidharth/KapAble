...
Replace the baked-in builtin language model catalog in `src/ipc/shared/language_model_constants.ts` with an API-first catalog fetched from `api.kapable.sh`, while preserving `language_model_constants.ts` as a local fallback when the API is unavailable or invalid.
...
- API-first: builtin provider/model metadata should come from `api.kapable.sh`.
...
- fetch from `https://api.kapable.sh/v1/language-model-catalog`
- validate with Zod
- cache in memory
...
- `kapable/theme-generator/default`
...
`GET https://api.kapable.sh/v1/language-model-catalog`
...
- `kapable/theme-generator/google` resolves to the concrete Google model to use for theme generation.
- `kapable/auto/openai` resolves to the concrete OpenAI model used in auto mode.
- `kapable/help-bot/default` resolves to the concrete model used by the help bot.
...
This removes the need for a dedicated `kapable/theme-generator/default` alias.
...
1. `kapable/auto/openai`
2. `kapable/auto/anthropic`
3. `kapable/auto/google`
...
- the KapAble Pro local-agent fallback also hardcodes exact concrete models

Planned change:

- resolve `kapable/auto/openai`
- resolve `kapable/auto/anthropic`
- resolve `kapable/auto/google`
...
- resolve `kapable/help-bot/default`
...
- Builtin cloud providers/models are fetched from `api.kapable.sh` when available.
