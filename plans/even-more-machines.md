...
`window.__kapableVersionPreviewLog`). Promote it: generic
`createTraceObserver(machineName)` in the kernel — 100-entry ring buffer,
applied + ignored events with reasons, `window.__kapableMachines` index — wired
...
- **Pro/billing gating, KapAble Pro auth return, help-bot streams,
...
maxEntries?, describeState?, describeEvent?, describeCommand?, mute? })`
     ...
     main-process access; `window.__kapableMachines` (index + `dump()`)
   ...
   - Decision: delete debug.ts and `window.__kapableVersionPreviewLog` outright
   ...
   - Exit: all convention-compliant machines emit to `window.__kapableMachines`;
     debug.ts gone.
...
downloadTimeoutSignal])`; (ii) `controller.signal.aborted` checks after
   ...
   (:210), throwing `KapableErrorKind.UserCancelled`; (iii) `activeControllers`
...
- [ ] trace.ts landed; version_preview/plan_handoff/app_run/connection_flow
      observable via `window.__kapableMachines`/`getTraceLog`; debug.ts deleted.
...
- **One generic machine, not a core with three instances.** The transition
  ...
  surfaces (ChatInput banner, QuestionnaireInput panel, KapableAddIntegration
...
- **NotFound semantics win.** integration_handlers.ts:14-22 is correct;
  ...
  `KapableErrorKind.NotFound`. Renderers show confirmation UI only on a
...
1. **GitHub structured error codes through the IPC envelope** (small).
   - Root cause, sharper than §4 recorded: the renderer's structured checks
     ...
     `KapableErrorKind` (:242-253), so `GitStateError.code`
