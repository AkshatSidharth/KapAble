...
- **Keep full-snapshot renderer writes to `.kapable/queue`.** The current store
...
- Legacy `.kapable/queue` migration stages and validates all attachment payloads
...
5. **Create main queue/intent persistence and acceptance transaction.** Migrate
   `.kapable/queue` entries once, paused, into the main aggregate. Keep the old
...
4. Legacy `.kapable/queue` files are imported once and intentionally retained
