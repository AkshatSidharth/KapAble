...
- When rebuilding local-agent history, append one provider-neutral synthetic
  ...
  - If `commitHash` exists, append
    `<kapable-git-context commit="FINAL_HASH"></kapable-git-context>`.
  - Otherwise, if `sourceCommitHash` exists, append
    `<kapable-git-context source_commit="START_HASH" no_commit="true"></kapable-git-context>`.
...
- Register the tools under
  ...
  through `src/components/chat/KapableMarkdownParser.tsx`, showing operation,
