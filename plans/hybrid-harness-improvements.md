...
`process.env.KAPABLE_ENGINE_URL` (and gateway URL) at module import;
...
`getKapableEngineUrl()` helper reading env per call). Audit for other
import-time-frozen env reads on the model-routing path (`KAPABLE_GATEWAY_URL`,
...
(`engine: true` → sets `KAPABLE_ENGINE_URL`/`KAPABLE_GATEWAY_URL` to the fake server
