...
"kapable Safe Storage" (post-`ready`) and "Chromium Safe Storage" (pre-`ready`
...
string | null`. **Synchronous. Never throws. Never shows UI. Returns null on
  ...
  per-ciphertext caching, stats, and the `KAPABLE_DISABLE_SAFE_STORAGE_RECOVERY`
...
user's machine were created **by the KapAble binary itself** (safeStorage created
the "Chromium"-named item too — the pre-ready race changes the _service name_,
not the creating app). So the signed KapAble binary is the trusted app in both
...
login-keychain password, while KapAble's main process is blocked in
`execFileSync`.

Corner case to keep in mind: a "Chromium Safe Storage" item created by some
_other_ app (e.g. an unbranded Chromium/Electron dev build) is NOT in KapAble's
...
- `KAPABLE_SAFE_STORAGE_READER=cli` reverts to v1 behavior (support escape hatch
...
- `KAPABLE_DISABLE_SAFE_STORAGE_RECOVERY=1` still short-circuits everything
...
- Respect `KAPABLE_SKIP_NATIVE_REBUILD` (`forge.config.ts:114`) the same way
...
`~/Library/Logs/kapable/main.log`. Also verify the `KAPABLE_SAFE_STORAGE_READER=cli`
...
- **Per-arch builds**: forge builds per-arch (`kapable-darwin-arm64`); the addon
