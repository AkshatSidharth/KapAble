...
KapAble currently treats an app's display name and filesystem folder name inconsistently. Some flows use the app name directly as a folder path, while app blueprint approval sanitizes the display name into a folder name before renaming.
...
- On blueprint approval, `KapableAppBlueprintCard` derives the target folder with `sanitizeAppFolderName(effectiveAppName)`.
...
As display names, KapAble currently allows:
...
The common path should stay simple: users type a nice app name, KapAble sanitizes it on their behalf, and KapAble creates a safe folder. New folder names should be lowercase slugs. User-entered and blueprint-generated display names should preserve readable/title-case formatting after sanitization, while folder names use lowercase slugs.
...
3. Apply the policy at every filesystem-writing app flow.
   ...
   - import-with-copy: same as create when copying into KapAble apps.
...
5. Improve errors.
   - Throw `KapableError` with `KapableErrorKind.Validation` for invalid folder names.
   - Keep conflict errors as `KapableErrorKind.Conflict`.
