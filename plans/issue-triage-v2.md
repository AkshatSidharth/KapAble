...
1. says plainly what we think is going on (a bug in KapAble, already fixed, an outside service, a setup problem, a problem in the app they built, or "we can't tell yet"),
...
| Not a KapAble problem (outage, provider, OS, the user's own app)   |      10 | #3959, #4172, #4250, #4359, #3974 | Partly (status feeds and playbook)                    |
...
Someone from the KapAble team will follow up here.

<details>
<summary>Notes for the KapAble team</summary>

- KapAble 1.9.0 (current 1.13.0), Windows, model auto:auto, Build mode
- Log signature: ...
...
Proposed title: `[session report] No description (KapAble 1.9.0, Windows)`

> Hi @reporter, thanks for sending this from KapAble.
>
> **What's going on:** Your report came through without a description, so we can't tell yet what went wrong. The logs show that KapAble's automatic update check failed, which is probably not what you noticed, but it does mean you're on an older version.
>
> **What you can do now:** Download the latest version from https://www.kapable.sh/download and try again. A lot has been fixed since 1.9.0.
>
...
> Someone from the KapAble team will follow up here.
>
> <details><summary>Notes for the KapAble team</summary>
>
...
> Hi @reporter, thanks for sending this from KapAble.
>
> **What's going on:** KapAble can't find Node.js on your computer, which it needs to run your app. This is a setup problem rather than something wrong with your app, and it has a known fix.
>
> **What you can do now:**
>
> 1. Install Node.js from https://nodejs.org (pick the LTS version).
> 2. Quit KapAble completely and open it again.
> 3. If it still doesn't work, open **Settings**, find **Node.js Path Configuration**, and click **Browse for Node.js** to point KapAble at the folder where you installed it.
>
...
> Hi @reporter, thanks for the report.
>
> **What's going on:** When your KapAble Pro credits run out, KapAble keeps trying to use Pro rather than switching to your own Anthropic key. That's how it works today, so nothing is wrong with your setup, and you can switch it yourself.
>
> **What you can do now:** Click the **Pro** button next to the model picker and turn off **Enable KapAble Pro**. KapAble will use your own API key until you turn it back on.
>
> Someone from the KapAble team will follow up here.
>
> <details><summary>Notes for the KapAble team</summary>
...
| -------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------- |
| Report has no description                                                        | Ask what they were doing and what they saw, plus a screenshot. Title it `[session report] No description (KapAble X, OS)`. Ignore the Auto-Updater Logs section unless the reporter mentions updating. If the version is behind, suggest updating. | 73 issues; #4408, #4393, #4263           |
| "node is not recognized" or pnpm not found (Windows)                             | Install Node.js from nodejs.org (LTS), quit and reopen KapAble; else **Settings** > **Node.js Path Configuration** > **Browse for Node.js**.                                                                                                       | #3665, #3348, #4188, #4189, #3612, #4456 |
...
| Pro credits ran out, daily chat limit, or wants to use own key                   | **Pro** button > turn off **Enable KapAble Pro**.                                                                                                                                                                                                  | #4392, #4388                             |
...
| Import fails after a crash, folder already exists                                | Delete or rename the folder in `kapable-apps`, then import again. Fixed in 1.10.0-beta.3.                                                                                                                                                          | #4217                                    |
...
| KapAble crashes or force closes                                                  | Update (stability work landed in 1.7.0). **Help** > **Upload Chat Session**. If logs are trimmed, email the `main` log from `%APPDATA%\kapable\logs`.                                                                                              | #3490, #4294, #3863, #3980               |
...
| Network loop or error inside the app the user built                              | That's the app, not KapAble. Switch to Agent mode and ask KapAble to find the cause.                                                                                                                                                               | #4359                                    |
...
- **Title**: when the description is empty, never derive a title from the Auto-Updater Logs section. Default to `[session report] No description (KapAble X, OS)` unless the main Logs show one clear user-facing failure (like Node.js missing), in which case name that.
- **Assessment** (new): one of `likely_kapable_bug`, `fixed_in_release`, `external_service`, `user_app_issue`, `environment_setup`, `needs_info`, `feature_request`, `question`, `needs_human`. Must cite evidence: a log line, a playbook entry, a release note, or a maintainer comment. No evidence means `needs_info`.
...
```json
{
  ...
  "summary": "KapAble can't find Node.js on your computer, which it needs to run your app.",
  "steps": [
...
- Sanitize all free text: strip HTML, strip @-mentions other than the reporter, allow links only to `github.com/AkshatSidharth/KapAble`, `www.kapable.sh`, `nodejs.org`, `githubstatus.com`, `status.supabase.com`; cap lengths (summary 400 chars, each step 200, notes 1200).
...
- If `triage.json` is missing, post a minimal safe comment ("Thanks for the report, someone from the KapAble team will take a look") and add a `triage/failed` label, so failures are visible on the issue instead of only in the daily health check.
