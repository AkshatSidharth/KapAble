# KapAble-managed Node.js runtime
...
- **Settings → General**: show active runtime ("Node v22.22.3 — KapAble-managed" / "… — System (`/opt/homebrew/bin/node`)"), the preference toggle, and "Remove managed Node.js" (deletes the dir, flips preference back to system). Full transparency defuses the "what did you install on my machine" objection.
...
- Version bumps ride normal KapAble releases (bump the manifest). On startup, if managed Node exists but ≠ pinned version: install the new version in the background, keep the old one until the new one passes verification, then delete the old. No self-updating outside app releases — app releases are the security-patch channel; Node security releases become "bump manifest + release" chores.
...
| Rosetta (x64 KapAble on arm64 Mac)                            | Match the app's arch (`os.arch()`); x64 Node under Rosetta works. No sysctl sniffing in v1                                                                                                                                                               |
...
1. **PR 1**: `managed_node.ts` + IPC + resolution wiring + settings field (no UI; dev-flag testable via a `KAPABLE_DEV_NODEJS_STATUS`-style override)
