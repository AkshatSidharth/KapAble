...
- Pro promo clicks open the in-app `KapableProTrialDialog` (no browser context switch); the dialog now takes a `utmCampaign` prop so checkout/learn-more traffic is attributed per message (`streaming-promo-<messageId>`).
...
- Shown only while `isStreaming` to non-Pro users (`!settings?.enableKapablePro && !userBudget`), then **disappears the moment the stream ends** — exactly when the user's attention returns to the chat.
...
- Messaging is feature-jargon-led ("Turbo Edits", "Smart Context") with no offer. Meanwhile the app _has_ a 7-day free trial (`trialCode=7PRO30`, used by KapableProTrialDialog/SetupBanner) that none of these messages mention.
...
4. **No offer, no proof, no numbers**: "Get KapAble Pro for faster edits with Turbo Edits" means nothing to someone who doesn't know what Turbo Edits is.
...
- Promo click → open existing `KapableProTrialDialog` (no context switch, stream keeps going).
...
- Lead with the trial: "Try KapAble Pro free for 7 days" beats "Get KapAble Pro".
...
- Sparkles/gradient accent consistent with KapableProTrialDialog, real button-styled CTA, dark-mode support, subtle fade-in.
...
## Link destination: kapable.sh/pro vs academy sign-in→checkout

- **Education-style messages** (user may not know what Pro is) → `https://www.kapable.sh/pro?utm_...` — consideration page, low bounce.
- **Direct-offer messages** ("Start your free 7-day trial") → `https://academy.kapable.sh/redirect-to-checkout?trialCode=7PRO30&utm_...` — same pattern the app already uses. Prefer this over hardcoding `/sign-in?redirect_url=...`: the redirect endpoint owns the auth flow (survives auth changes) and carries the trial code.
- **Best of both**: click → in-app `KapableProTrialDialog`, which offers "Start Free Trial" (checkout) and "Learn more" (/pro). Warm the user before the sign-in wall.
