...
`messages.content` is the source of user-visible chat data, but assistant messages can contain full files, SQL, diffs, logs, schemas, and tool results inside KapAble tags. Build and index a deterministic projection designed specifically for chat recall.
...
- Missing and cross-app chat/message IDs both produce `KapableErrorKind.NotFound`, avoiding cross-app existence disclosure.
...
Preserve user-authored text. Do not interpret literal `<kapable-*>` examples in user messages as trusted tool markup. Apply only normalization and a generous pathological-size bound.

### Assistant messages

Use the existing structured streaming-message parser where practical rather than a generic `<kapable-*>...</kapable-*>` regex. Apply an explicit policy by block/tag class:
...
- Drop the bodies of `kapable-search-chats` and `kapable-read-chat` so retrieved history never becomes recursively searchable copied history.
- For a newly introduced recognized KapAble tag without an explicit policy, fail closed by omitting its body and keeping only allowlisted short attributes. Add a test whenever a new tag becomes intentionally searchable.

Compaction summaries require explicit handling: the `<kapable-compaction>` body is high-signal searchable text. Original messages remain in the database, so post-ranking grouping should avoid returning a compaction-summary excerpt that merely duplicates a stronger original-message hit. Apply a small score penalty to summary rows while still allowing them to surface unique terms.
...
- Emit a `<kapable-search-chats>` card showing the query, index status, matched chat titles, dates, and excerpts consulted by the agent.
...
- Emit a `<kapable-read-chat>` card that shows the source chat, returned time/message range, and the bounded text actually consulted by the agent.
...
3. Add `kapable-search-chats` and `kapable-read-chat` to `KAPABLE_CUSTOM_TAG_NAMES` in `src/lib/streamingMessageParser.ts`; registering only React components is insufficient.
4. Add renderer components (shared where sensible) and register them in `KapableMarkdownParser.tsx`.
...
- User text is preserved even when it contains literal KapAble-tag examples.
...
- Prior `kapable-search-chats`/`kapable-read-chat` bodies are removed.
- Unknown assistant KapAble tags fail closed.
...
Use `/kapable:lint` when available. If an E2E test is added or run, execute `npm run build` first.
