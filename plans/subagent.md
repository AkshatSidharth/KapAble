...
KapAble will add a visible, root-controlled, depth-one sub-agent system with three fixed personas:
...
- Concurrency is not initially user-configurable and spawning does not require a separate confirmation. The initial release has no KapAble-defined per-child or aggregate token, step, tool-call, or wall-time limits; usage remains visible in the UI.
...
- If a persona's required model is unavailable, the run is blocked with setup guidance. KapAble does not silently substitute the root model or another provider model.
...
Both selected models exist in the reviewed Codex model catalog but are not currently registered in KapAble. Adding them to KapAble's model catalog/constants is therefore an implementation prerequisite. Resolve the exact persona model before scheduling; if it is unavailable for the Pro user's configured account/provider, block the run with setup guidance.

## Problem Statement

KapAble's Local Agent currently performs investigation, implementation, and self-review in one linear loop. Reconnaissance consumes root context, independent work cannot proceed concurrently, and users lack an explicit independent review checkpoint.

KapAble already has a specialized `explore_code` nested loop, but it is blocking, single-purpose, hard-coded, and lacks a durable lifecycle. This plan replaces that surface with a general child runtime while preserving KapAble's consent model, provider abstraction, live change visibility, and root-owned completion flow.

> KapAble can assemble a small, visible team of specialized agents while the root remains accountable for the overall task.
...
- Work through KapAble's Vercel AI SDK provider abstraction while requiring the exact persona defaults and clearly blocking unavailable configurations.
...
1. `enableExplorerSubagent?: boolean`
   ...
   - Copy: "Let KapAble automatically delegate read-only codebase research to Explorer. This may use additional model tokens."
   - Default: `true`.
...
- The initial release sets no KapAble-defined hard caps on child steps, tool calls, output tokens, wall time, or aggregate usage beyond provider/platform constraints.
...
Add `gpt-5.6-luna` and `gpt-5.6-sol` to KapAble's model constants/catalog with accurate capabilities. Resolve the exact persona model/provider before durable scheduling and record the provider, model, and reasoning on the thread. If the required model is unavailable, do not create a runnable child or substitute another model; return a handled blocked result with setup guidance.
...
- [ ] Add `gpt-5.6-luna` and `gpt-5.6-sol` to KapAble model metadata and implement exact-model blocking with setup guidance.
...
- No KapAble-defined token, step, tool-call, wall-time, or aggregate usage caps are imposed initially; measured usage remains visible.
...
- **Bridge, Don't Replace:** sub-agents reuse KapAble's workspace, tools, providers, diffs, settings, and consent model.
