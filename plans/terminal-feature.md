...
`npm run dev` survives chat switches. PTYs are killed when KapAble quits.

## Problem Statement

KapAble's target user — the prosumer/developer building real apps inside KapAble —
...
terminal, `cd` into a path they may not even know (KapAble resolves it
internally from `app.path`), and lose the in-KapAble flow.

Strategically, this is also KapAble's biggest credibility gap with VS Code /
...
- **CWD = `getKapableAppPath(app.path)`** for the chat's bound app.
- **PTY is per-app, not per-chat.** Switching chats within the same app
  reuses the same live PTY (your `npm run dev` doesn't die).
- **PTY lifecycle**: alive until KapAble quits or user explicitly kills it.
...
- **Theme reactivity**: xterm palette derived from KapAble's current light/dark
...
- **`prefers-reduced-motion`**: replaces slide-up with a 120ms crossfade.
  ...
  from framer-motion directly) that other KapAble animations can adopt later.
...
- Per-app user-configurable shell setting (use `$SHELL` for v1; add setting
  in v1.1).
- Scrollback persistence to disk across KapAble restarts.
...
- Detached / tmux-style session that survives KapAble quit.

## User Stories

- **As a KapAble developer**, I want to open a terminal at my app's path with
  one click so I can run git/npm/CLIs without leaving KapAble or remembering
  the path.
...
- **As a vim/htop user**, I want pressing Esc inside the terminal to behave
  normally (escape insert mode, etc.) and NOT exit KapAble's terminal mode.
...
4. The escape banner fades in at the top of the terminal region:
   ...
   adapted to OS).
5. A subtle initializing placeholder (KapAble's existing loading orb +
...
- **Initializing**: 50–200ms window between toggle-on and PTY-ready.
  Placeholder + KapAble orb + scramble verb. Banner already visible.
...
- **Switching chats within same app, both terminal-on**: the terminal
  ...
  in the banner does a scramble-text reveal (KapAble signature). Same PTY
...
- **Escape banner**: a slim (~32px) full-width bar at the top of the
  ...
  consistent with the rest of KapAble.
...
- Contrast: banner text meets WCAG AA against both KapAble themes.
...
- Empty-state cards match KapAble's existing rounded-2xl bordered card +
...
is keyed by app and lives until KapAble quits or the user explicitly kills
...
- **Spawn**: lazy, on first terminal-open for an app. Uses the existing
  ...
  `getKapableAppPath(app.path)` — validated server-side as an absolute,
...
- **Survives KapAble quit**: NO. All PTYs killed on `before-quit` (via the
...
3. **CWD is server-side resolved**. Renderer cannot pass an arbitrary
   cwd. Always `getKapableAppPath(app.path)`, validated absolute + exists.
...
- **E2E (Playwright)** — the one that proves it actually works:
  - Smoke: open chat with app, click terminal toggle, type
    `echo hello kapable\n`, assert "hello kapable" appears via
...
- [ ] `TerminalPanel.tsx` (lazy-loaded). xterm + all addons. Theme from
      KapAble theme atom. `screenReaderMode: true`. Resize via
...
- **PTY survives chat switch across apps; killed only on KapAble quit or
...
- PTY lifecycle → keep alive across chat switches, kill on KapAble quit.
...
_Generated by kapable:swarm-to-plan (PM, UX, Eng + Will)_
