...
- Delete the Build-mode repair loop in `chat_stream_handlers.ts`, including speculative type checks, `<kapable-problem-report>` generation, retry prompts, and the virtual codebase assembled from pending write/rename/delete tags. Preserve parsing/rendering and transcript cleanup for `<kapable-problem-report>` so historical chats continue to display and replay safely.
...
- Preserve config selection order: `tsconfig.app.json`, then `tsconfig.json`. Run the local CLI from `appPath` with fixed arguments equivalent to:
  ...
  ```text
  tsc --pretty false --noEmit --incremental --tsBuildInfoFile <kapable-cache-file> --project <config>
  ```

  Store build info under KapAble’s TypeScript cache, keyed by app path, config path, and local TypeScript version, so checks never write generated files into the user project and caches are not shared across compiler versions.
...
- Centralize Code Explorer compiler resolution in the worker:
  ...
  4. Use the local module when compatible; if loading fails or the required API is missing, load KapAble’s packaged `@typescript/typescript6` module and log the local version/reason and fallback version.
...
- Type checks use the app-local `tsc`, including native TypeScript 7, and never fall back to KapAble TS6.
