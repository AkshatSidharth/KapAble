...
1. `kapable-{version}-1.x86_64.rpm` (Linux RPM)
2. `kapable-{version}-full.nupkg` (Windows NuGet package)
3. `kapable-{version}.Setup.exe` (Windows installer)
4. `kapable-darwin-arm64-{version}.zip` (macOS Apple Silicon)
5. `kapable-darwin-x64-{version}.zip` (macOS Intel)
6. `kapable_{version}_amd64.deb` (Linux DEB)
