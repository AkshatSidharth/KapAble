...
//
// Example:
//   node scripts/resolve-crash-frames.mjs kapable/DF23C4907A7C231BEA2D1962BED725290/0x6ed35df
//
...
//
// Installed builds rename Electron's binaries to "kapable", while the
...
const symDir = path.join(userCacheDir(), "kapable-symbol-cache", "sym");
...
// their reported name, so /kapable/i names retry as the Electron names for
...
async function ensureSymbols(debugFile, debugId) {
  ...
  if (/kapable/i.test(debugFile)) {
    candidates.push(
      debugFile.replace(/kapable/gi, "electron"),
      debugFile.replace(/kapable/gi, "Electron"),
    );
  }
