...
All models are routed through the KapAble Engine gateway, so you only need one
credential: a KapAble Pro API key, exposed as `KAPABLE_PRO_API_KEY`.

The suite is skipped entirely when `KAPABLE_PRO_API_KEY` is unset — no tests will
...
```bash
KAPABLE_PRO_API_KEY="..." EVAL_SUITE=all EVAL_MODEL=all npm run eval
```

Optional: override the gateway URL with `KAPABLE_ENGINE_URL` (defaults to
...
EVAL_SUITE=all EVAL_MODEL=all KAPABLE_PRO_API_KEY="..." npm run eval
```
...
EVAL_SUITE=search_replace EVAL_MODEL=all KAPABLE_PRO_API_KEY="..." npm run eval

# The basic_agent suite (Basic agent prompt, search_replace + write_file)
EVAL_SUITE=basic_agent EVAL_MODEL=all KAPABLE_PRO_API_KEY="..." npm run eval

# The pro_agent suite (Pro agent prompt, search_replace + write_file)
EVAL_SUITE=pro_agent EVAL_MODEL=all KAPABLE_PRO_API_KEY="..." npm run eval
```
...
```bash
EVAL_SUITE=all EVAL_MODEL=all KAPABLE_PRO_API_KEY="..." \
...
```bash
EVAL_SUITE=all EVAL_MODEL=all KAPABLE_PRO_API_KEY="..." npm run eval -- -t "zod"
```
...
```bash
EVAL_SUITE=all EVAL_MODEL=sonnet KAPABLE_PRO_API_KEY="..." npm run eval
```
...
EVAL_SUITE=search_replace EVAL_MODEL=sonnet \
  KAPABLE_PRO_API_KEY="..." npm run eval -- -t "Extract a helper function"
```
...
```bash
KAPABLE_PRO_API_KEY="..." EVAL_SUITE=newline_probe EVAL_MODEL="GPT 5.6 Sol" npm run eval
```
