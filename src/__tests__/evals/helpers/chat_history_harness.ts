...
const CURRENT_ARM_SYSTEM = (
  appName: string,
) => `You are the AI assistant inside KapAble, an app builder. You are working with the user on their app "${appName}". You have tools to search and read this app's OTHER (historical) chats.
...
const SUBAGENT_ANSWER_SYSTEM = (
  appName: string,
) => `You are the AI assistant inside KapAble, working with the user on their app "${appName}". You delegated historical research to a read-only sub-agent; its validated report is provided. Answer the user's question from the report alone.
...
const CONTROL_SYSTEM = (appName: string) =>
  `You are the AI assistant inside KapAble, working with the user on their app "${appName}". Answer the user's question about prior work or decisions from what you know. You have no access to past conversations, so if you do not actually know, say plainly that you have no record — never guess or invent decisions.`;
