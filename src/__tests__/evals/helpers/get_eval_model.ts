...
// because KapAble's production picker does not currently surface GPT 5.4 —
// it had refusal/routing issues — but the eval harness still uses it as
// the judge model.
export const GPT_5_4 = "gpt-5.4";

// Single source of truth for the KapAble Engine URL across the eval helpers.
export const KAPABLE_ENGINE_URL =
...
export function hasKapableProKey(): boolean {
  return !!process.env.KAPABLE_PRO_API_KEY;
}

let _provider: KapableEngineProvider | null = null;
 ...
 *
 * The KapAble Engine only supports `stream: true`, but the AI SDK sends
...
/**
 * Fetch wrapper that adapts requests for the KapAble Engine, which only supports
...
const evalFetch: typeof fetch = async (input, init) => {
  ...
  // Force streaming — the KapAble Engine returns 500 for non-streaming requests
  parsed.stream = true;
...
function getProvider(): KapableEngineProvider {
  if (!_provider) {
    _provider = createKapableEngine({
      apiKey: process.env.KAPABLE_PRO_API_KEY,
      baseURL: KAPABLE_ENGINE_URL,
      kapableOptions: {
...
export function getEvalModel(
  ...
  return kapableProvider(modelId, { providerId: provider });
}
