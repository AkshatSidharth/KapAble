...
const KAPABLE_LOGS_SCRIPT = readFileSync(
  resolve(__dirname, "../../worker/kapable_logs.js"),
  "utf8",
);
...
function loadConsoleInterceptor() {
  ...
  runInNewContext(KAPABLE_LOGS_SCRIPT, {
...
describe("kapable console interception", () => {
  ...
  it("preserves up to 50 object keys before marking additional keys", () => {
    ...
    expect(serialized.__kapable_truncated__).toBe("Additional keys omitted");
  });
