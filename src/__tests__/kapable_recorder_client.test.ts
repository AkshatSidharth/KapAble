...
const recorderSource = fs.readFileSync(
  path.resolve(process.cwd(), "worker/kapable-recorder-client.js"),
  "utf8",
);
...
function setup({ allowUntrusted = true }: { allowUntrusted?: boolean } = {}) {
  ...
  Object.defineProperty(doc, "currentScript", {
    configurable: true,
    value: {
      dataset: { kapableRecorderToken: RECORDER_TOKEN },
  ...
  const win: any = {
    ...
    __KAPABLE_RECORDER_ALLOW_UNTRUSTED__: allowUntrusted,
  ...
  if (!messages.some((m) => m.type === "kapable-recorder-initialized")) {
  ...
  const activate = (token = RECORDER_TOKEN) =>
    messageHandler!({
      source: parent,
      data: { type: "activate-kapable-recorder", token },
    });
  const deactivate = () =>
    messageHandler!({
      source: parent,
      data: { type: "deactivate-kapable-recorder", token: RECORDER_TOKEN },
    });
  ...
  const overlays = () => doc.querySelectorAll(".__kapable_recorder_overlay__");
...
describe("kapable recorder client", () => {
  ...
  it("takes the capability out of the DOM once it has read it", () => {
    ...
    expect(r.removedAttributes).toContain("data-kapable-recorder-token");
  ...
  it("acknowledges a flush after actions already sent to the parent", () => {
    ...
    expect(r.messages.slice(-2)).toEqual([
      expect.objectContaining({ type: "kapable-recorder-action" }),
      { type: "kapable-recorder-flushed", requestId: "flush-1" },
    ]);
  });
  ...
  it("uses data-dyad-id only as a source hint for a CSS fallback", () => {
    const r = setup();
    // The attribute is a source location injected only by KapAble's dev plugin, so
  ...
  it("cannot be switched into accepting untrusted events after load", () => {
    ...
    const r = setup({ allowUntrusted: false });
    r.win.__KAPABLE_RECORDER_ALLOW_UNTRUSTED__ = true;
    r.setHtml(`<button>Add</button>`);
    r.activate();
    r.click(r.doc.querySelector("button"));
    r.settleClick();
  ...
  it("removes the escape-hatch global once it has been read", () => {
    const r = setup();
    expect(r.win.__KAPABLE_RECORDER_ALLOW_UNTRUSTED__).toBeUndefined();
  });
});
