...
/**
 * The way into a server KapAble set up.
 *
 * KapAble invents the admin password and mints the API token, so it is the only
 ...
 * making them click to discover that KapAble even has them means most people
 ...
 * the absence is KapAble holding one it cannot open — not a server that never
...
function LockedPassword() {
  return (
    ...
    >
      KapAble is holding an admin password for this server but cannot read it on
      this machine.
    </p>
  );
}
...
export function CoolifyCredentials({
  ...
  // blank where those belong reads as KapAble holding nothing rather than as a
  ...
  if (isPending && !isError) {
    return (
      ...
      >
        Looking up what KapAble has stored…
      </p>
    );
  }
  ...
  // retry — still leaves them readable, and taking a password KapAble holds the
  ...
  if (isError && !credentials) {
    return (
      ...
      >
        KapAble could not read what it has stored for this Coolify.
      </p>
    );
  }
  ...
  // The usual case: KapAble set the server up and is connected to it. Merged on
  ...
  return (
    <div className="space-y-2 text-sm" data-testid="coolify-credentials">
      ...
      {/* A password KapAble holds but cannot decrypt comes back absent, which
        ...
        <>
          ...
          {instance && (
            ...
            >
              {showsBoth && (
                <div className="text-muted-foreground text-xs">
                  The Coolify KapAble is connected to
                </div>
              )}
              <Field
                label="Address"
