 ...
 * step is the only manual one: nothing can reach the server until KapAble's key is
 * on it. Everything after that is KapAble's job, and the panel's remaining work is
  ...
  return (
    <Button
      ...
      onClick={() => {
        ...
        // only copy of a password KapAble invented, and a button that quietly
...
export function CoolifyServerSetup({
  ...
  /**
   * The server KapAble already holds an account for, if it holds one.
   *
   * Installing is refused outright while it does — KapAble has the only copy of
  ...
  const inspect = useMutation<SetupPreflight, Error, void>({
    mutationFn: async () => {
      ...
      // The screen's premise is that KapAble installs onto the machine that just
  ...
  if (setup.type === "done") {
    ...
    return (
      <div className="space-y-3" data-testid="coolify-setup-done">
        ...
        {/* KapAble keeps these, so this is a copy rather than the only sight of
        ...
        {/* Only when it is true. KapAble asks for a certificate and usually gets
        ...
        {!result.secure && (
          ...
          >
            ...
            {/* A decision rather than a notice, and only where there is one to
                ...
                this leaves KapAble unconnected rather than connected over a
            ...
            {result.tokenStored && (
              <label className="mt-2 flex items-center gap-2">
                ...
                />
                <span>Keep KapAble connected to this address anyway</span>
              </label>
            )}
          </div>
        )}
        {result.tokenStored && (
          <p className="text-sm text-muted-foreground">
            {result.secure
              ? "KapAble created its own API token, so you can pick a server and project next."
              : "KapAble created an API token for this server. It is not kept unless you say so above, because this address is not encrypted."}
          </p>
        )}
        {(!result.tokenStored ||
          (!result.secure && !acceptedInsecureToken)) && (
          // The install stands; only the last step did not. Saying so plainly
          // beats implying the whole thing failed. A token KapAble made but will
          ...
          >
            ...
            <p className="text-muted-foreground">
              {result.tokenStored
                ? "Unless you tick the box above, KapAble forgets the token it made."
                : (result.tokenUnavailableReason ??
                  "KapAble could not create an API token automatically.")}{" "}
              Open {result.dashboardUrl}, sign in with the details above,{" "}
              {/* KapAble turns the API on before it mints, so this stays done
    ...
    <div className="space-y-3" data-testid="coolify-server-setup">
      ...
      <div className="space-y-1">
        ...
        <p className="text-xs text-muted-foreground">
          ...
          page. Paste this in there and the server will trust KapAble from the
          moment it starts.
        </p>
        <p className="text-xs text-muted-foreground">
      ...
      <div>
        ...
        {/* Checked while typing, because neither reason is cheap to find out
            ...
            an address KapAble cannot put in a shell word costs a run that
      ...
      <div>
        ...
        {/* KapAble can get a certificate without this, using a free service that
      ...
      {inspectionForHost && (
        ...
        >
          {inspectionForHost.hostFingerprint && (
            ...
            // provider's console before KapAble sends anything to the machine.
            <div className="flex items-center justify-between gap-2">
      ...
      {snapshot.isError && (
        ...
        >
          Could not read what KapAble is doing with servers right now.{" "}
          <button
            type="button"
      ...
      <div className="flex gap-2">
        ...
        <Button
          disabled={
            ...
            // Refused by the handler while KapAble holds an account, so offering
