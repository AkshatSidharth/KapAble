...
describe("acknowledging the loss", () => {
  it("will not sign out until the box is ticked", async () => {
    ...
    // KapAble has the only copy, so confirming has to be a separate act.
    await openAndSettle();
...
describe("nothing to look at yet", () => {
  ...
  it("says it is looking only once", async () => {
    ...
    expect(
      await screen.findAllByText(/Looking up what KapAble has stored/),
    ).toHaveLength(1);
  });
  ...
  it("says when it holds a password it cannot read", async () => {
    ...
    // Said once. The panel below states what KapAble is holding; this only adds
...
describe("the last look", () => {
  ...
  it("does not say it for an instance KapAble did not set up", async () => {
    // Connected by pasting a token, so nothing here was invented by KapAble and
