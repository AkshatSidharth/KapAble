...
} from "./KapableMarkdownParser";
import { KapableAttachment, type AttachmentSize } from "./KapableAttachment";
...
/** Extract <kapable-attachment> tags from message content and return parsed attachment data. */
function extractAttachments(content: string): {
  ...
  const tagRegex = /<kapable-attachment\s+([^>]*)><\/kapable-attachment>/g;
  const attrRegex = /([\w-]+)="([^"]*)"/g;
  const results: {
    name: string;
    type: string;
...
/** Strip <kapable-attachment> tags from user message content. */
function stripAttachmentInfo(content: string): string {
  return content
    .replace(/<kapable-attachment\s+[^>]*><\/kapable-attachment>/g, "")
    .trim();
}
