    ...
    t: (key: string) =>
      ({
        ...
        "preview.fixWithAiDescription":
          "KapAble will try to fix the reported issues.",
        "preview.startingAiFix": "Opening chat...",
        "preview.checkingAiFixAvailability":
          "Checking whether KapAble can fix this...",
        "preview.fixWithAiToolDenied":
          "KapAble needs the run_pre_commit tool to fix this.",
...
describe("CommitCheckFailureAlert", () => {
  ...
  it("describes the AI fix as an attempt rather than something already done", () => {
    ...
    expect(alert).toContain("KapAble will try to fix the reported issues.");
  });
  ...
  it.each([
    ["tool-permission", "KapAble needs the run_pre_commit tool to fix this."],
