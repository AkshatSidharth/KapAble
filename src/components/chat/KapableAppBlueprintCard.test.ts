...
const templates: Template[] = [
  ...
  {
    id: "kapable-sh/community-template",
  ...
  {
    id: "kapable-sh/selected-community-template",
  ...
  it("keeps the current community template visible", () => {
    expect(
      getAppBlueprintTemplateOptions(
        templates,
        "kapable-sh/selected-community-template",
      ).map((template) => template.id),
    ).toEqual(["react", "kapable-sh/selected-community-template"]);
  });
});
