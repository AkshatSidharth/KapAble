...
/**
 * Accent color configuration for KapableCard components.
 * Maps to Tailwind color classes for border, background, and text.
 */
export type KapableAccentColor =
  | "blue"
  | "purple"
  | "violet"
  | "red"
...
export const KapableCardPresentationContext = createContext<
  "standalone" | "embedded"
>("standalone");

const ACCENT_BORDER: Record<KapableAccentColor, string> = {
...
const ACCENT_ICON_BG: Record<KapableAccentColor, string> = {
...
const ACCENT_BADGE: Record<KapableAccentColor, string> = {
...
interface KapableCardProps {
  children: React.ReactNode;
  state?: CustomTagState;
  accentColor?: KapableAccentColor;
  showAccent?: boolean;
...
/**
 * Premium container for all KapAble markdown action cards.
...
export function KapableCard({
  ...
  const presentation = useContext(KapableCardPresentationContext);
...
interface KapableCardHeaderProps {
...
/**
 * Header row for KapableCard. Contains a tinted icon circle and flexible content area.
 */
export function KapableCardHeader({
  icon,
  accentColor = "blue",
  children,
}: KapableCardHeaderProps) {
  return (
...
interface KapableBadgeProps {
  children: React.ReactNode;
  color?: KapableAccentColor;
}

/**
 * Small pill badge for labeling card types (e.g. "GREP", "Turbo Edit", "SQL").
 */
export function KapableBadge({ children, color = "blue" }: KapableBadgeProps) {
  return (
    <span
...
interface KapableExpandIconProps {
  isExpanded: boolean;
}
 ...
 */
export function KapableExpandIcon({ isExpanded }: KapableExpandIconProps) {
  return (
    <ChevronRight
      size={16}
...
interface KapableStateIndicatorProps {
  state: CustomTagState;
  pendingLabel?: string;
  warningLabel?: string;
  abortedLabel?: string;
 ...
 */
export function KapableStateIndicator({
  state,
  pendingLabel,
  warningLabel,
  abortedLabel,
 ...
 */
export function KapableFinishedIcon() {
  return (
...
interface KapableCardContentProps {
  isExpanded: boolean;
  children: React.ReactNode;
  className?: string;
}
 ...
 */
export function KapableCardContent({
  isExpanded,
  children,
  className = "",
}: KapableCardContentProps) {
...
interface KapableFilePathProps {
  path: string;
}
 ...
 */
export function KapableFilePath({ path }: KapableFilePathProps) {
  if (!path) return null;
  return (
...
interface KapableDescriptionProps {
  children: React.ReactNode;
}
 ...
 */
export function KapableDescription({ children }: KapableDescriptionProps) {
  return (
