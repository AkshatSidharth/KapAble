...
interface KapableCodebaseContextProps {
...
export const KapableCodebaseContext: React.FC<KapableCodebaseContextProps> = ({
  node,
}) => {
  ...
  return (
    ...
    >
      <KapableCardHeader icon={<Code2 size={15} />} accentColor="blue">
        <KapableBadge color="blue">Codebase Context</KapableBadge>
        {files.length > 0 && (
          <span className="text-xs text-muted-foreground">
            Using {files.length} file{files.length !== 1 ? "s" : ""}
          </span>
        )}
        {inProgress && <KapableStateIndicator state="pending" />}
