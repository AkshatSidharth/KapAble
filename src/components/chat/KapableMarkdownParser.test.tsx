...
// content string it received. The KapableMarkdownParser wraps ReactMarkdown
...
import { KapableMarkdownParser } from "./KapableMarkdownParser";

describe("KapableMarkdownParser kapable-status", () => {
  afterEach(() => {
  ...
  it("honors explicit aborted state on closed status tags", () => {
    render(
      <KapableMarkdownParser
        content={
          '<kapable-status title="Supabase functions failed" state="aborted">\n0 succeeded\n1 failed\n</kapable-status>'
        }
      />,
    );
  ...
  it("renders incomplete type checks as amber warnings", () => {
    const { container } = render(
      <KapableMarkdownParser
        content={
          '<kapable-status title="Type check incomplete" state="warning">Fix the configuration error, then rerun type checking.</kapable-status>'
        }
      />,
    );
  ...
  it("keeps completed checks with source errors in the green finished state", () => {
    const { container } = render(
      <KapableMarkdownParser
        content={
          '<kapable-status title="Type errors found" state="finished">Found 1 type error.</kapable-status>'
        }
      />,
    );
...
describe("KapableMarkdownParser kapable-command", () => {
  afterEach(() => {
    cleanup();
  });
  ...
  it("renders the add TypeScript command as a supported action button", () => {
    render(
      <KapableMarkdownParser
        content={'<kapable-command type="add-typescript"></kapable-command>'}
      />,
    );
...
describe("KapableMarkdownParser kapable-explore-chat-history", () => {
  afterEach(() => {
    cleanup();
  });
  ...
  it.each(["partial", "no_match"])(
    ...
    (outcome) => {
      render(
        <KapableMarkdownParser
          content={`<kapable-explore-chat-history query="auth" chats="1" evidence="2" outcome="${outcome}">Historical evidence</kapable-explore-chat-history>`}
        />,
      );

      expect(
        screen
          .getByTestId("kapable-explore-chat-history")
...
describe("KapableMarkdownParser kapable-git", () => {
  afterEach(() => {
    cleanup();
  });
  ...
  it.each([
    ...
    render(
      <KapableMarkdownParser
        content={`<kapable-git operation="${operation}" revision="abc123" path="src/main.ts"></kapable-git>`}
      />,
    );
  ...
  it("renders a pending state while the Git tag streams", () => {
    ...
    render(
      <Provider store={store}>
        <KapableMarkdownParser
          content={'<kapable-git operation="status" state="pending">'}
        />
      </Provider>,
    );
  ...
  it("shows a useful status summary and expands structured file details", () => {
    ...
    render(
      <KapableMarkdownParser
        content={`<kapable-git operation="status" branch="main" changed_count="2" untracked_count="1" conflicted_count="0" detail_format="status">${status}</kapable-git>`}
      />,
    );
  ...
  it("does not render malformed status detail as structured content", () => {
    render(
      <KapableMarkdownParser
        content={
          '<kapable-git operation="status" detail_format="status">{"branch":"main","detached":false,"staged":[{}],"unstaged":[],"untracked":[],"conflicted":[]}</kapable-git>'
        }
      />,
    );
...
describe("KapableMarkdownParser kapable-security-finding", () => {
  afterEach(() => {
    cleanup();
  });
  ...
  it("renders a card with the title and severity level", () => {
    render(
      <KapableMarkdownParser
        content={
          '<kapable-security-finding title="SQL Injection in User Lookup" level="critical">\nUser input is concatenated into a query.\n</kapable-security-finding>'
        }
      />,
    );
  ...
  it("falls back gracefully when the level is missing or invalid", () => {
    render(
      <KapableMarkdownParser
        content={'<kapable-security-finding title="Partial finding">'}
      />,
    );
...
describe("KapableMarkdownParser closed-block render counts", () => {
  beforeEach(() => {
  ...
  // Three markdown segments separated by two closed kapable-status tags.
  ...
  it("renders each markdown block exactly once for a one-shot parse", () => {
    render(<KapableMarkdownParser content={FULL} />);
  ...
  it("does not re-render closed markdown blocks as later content streams in", () => {
    ...
    const { rerender } = render(
      <KapableMarkdownParser content={FULL.slice(0, md1ClosesAt)} />,
    );
    ...
    for (let i = md1ClosesAt + 1; i <= md2ClosesAt; i++) {
      rerender(<KapableMarkdownParser content={FULL.slice(0, i)} />);
    }
...
describe("KapableMarkdownParser parser-cache perf metrics", () => {
  ...
  function streamFully() {
    const { rerender } = render(
      <KapableMarkdownParser content={FULL.slice(0, 1)} />,
    );
