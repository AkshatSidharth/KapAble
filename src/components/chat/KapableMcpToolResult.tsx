...
interface KapableMcpToolResultProps {
  node?: any;
  children?: React.ReactNode;
}

export const KapableMcpToolResult: React.FC<KapableMcpToolResultProps> = ({
  node,
  children,
}) => {
  ...
  const prettyJson = useMemo(() => {
    ...
    try {
      ...
      console.error("Error parsing JSON for kapable-mcp-tool-result", e);
      return raw;
    }
  }, [expanded, raw]);
  ...
  return (
    ...
    >
      <KapableCardHeader icon={<CheckCircle size={15} />} accentColor="emerald">
        <KapableBadge color="emerald">Tool Result</KapableBadge>
        {serverName && (
        ...
        <div className="ml-auto">
          <KapableExpandIcon isExpanded={expanded} />
        </div>
      </KapableCardHeader>
      <KapableCardContent isExpanded={expanded}>
