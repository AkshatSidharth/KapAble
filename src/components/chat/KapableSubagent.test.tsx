...
function makeActivity(): SubagentActivity {
  return {
    ...
    status: "completed",
    presentationXml:
      '<kapable-grep query="auth" count="1">src/auth.ts:1: auth</kapable-grep>',
    error: null,
    startedAt: new Date(),
...
describe("KapableSubagent", () => {
  ...
  it("shows live tool activity and collapses into findings when complete", async () => {
    ...
    expect(await screen.findByText(/kapable-grep state/)).toBeTruthy();
    ...
    await waitFor(() => {
      expect(screen.queryByText(/kapable-grep state/)).toBeNull();
      expect(
