...
import {
  KapableCard,
  KapableCardHeader,
  KapableCardPresentationContext,
} from "./KapableCardPrimitives";

interface KapableSubagentProps {
  chatId: number;
  threadId: string;
  persona: string;
  taskName: string;
...
export function KapableSubagent({
  chatId,
  threadId,
  persona,
  taskName,
  renderActivity,
}: KapableSubagentProps) {
  ...
  return (
    <KapableCard className="overflow-hidden">
      <KapableCardHeader icon={<Bot size={15} />} accentColor="indigo">
        <button
          type="button"
      ...
      {isExpanded && (
        <div className="border-t border-border/60 px-3 py-2">
          {loadError ? (
            ...
            <KapableCardPresentationContext.Provider value="embedded">
            ...
            </KapableCardPresentationContext.Provider>
          ) : isActive ? (
            <p className="py-2 text-xs text-muted-foreground">
...
function withActivityState(
  ...
  return xml.replace(/^(<kapable-[\w-]+)/, `$1 state="${state}"`);
}
