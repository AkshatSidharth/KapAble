...
describe("TestAssertionsPlanCard", () => {
  it("resumes the parked turn instead of sending a chat message", async () => {
    ...
    screen.getByTestId("kapable-test-assertions-approve-button").click();
  ...
  it("closing the card answers the parked turn with no spec", async () => {
    ...
    screen.getByTestId("kapable-test-assertions-discard-button").click();
    ...
    await screen.findByTestId("kapable-test-assertions-discarded-note");
  });
  ...
  it("leaves the plan approvable when the discard can't be recorded", async () => {
    ...
    expect(
      screen.queryByTestId("kapable-test-assertions-discarded-note"),
    ).toBeNull();
  });
  ...
  it("does not start a second turn when the hand-off fails but the turn is still parked", async () => {
    ...
    await screen.findByTestId("kapable-test-assertions-open-file-button");
    expect(mocks.streamMessage).not.toHaveBeenCalled();
    expect(mocks.syncChatFromDb).not.toHaveBeenCalled();
  });
...
describe("TestAssertionsInput", () => {
  it("puts up the plan the transcript says is still waiting on a review", () => {
    ...
    expect(screen.getByTestId("kapable-test-assertions-card")).toBeTruthy();
    expect(screen.getByTestId("kapable-test-assertions-step-0")).toBeTruthy();
    expect(
      screen.getByTestId("kapable-test-assertions-approve-button"),
    ).toBeTruthy();
  });
  ...
  it.each(["approved", "discarded"] as const)(
    ...
    (status) => {
      ...
      expect(screen.queryByTestId("kapable-test-assertions-card")).toBeNull();
    },
  );
  ...
  it("puts the newest unanswered plan on top and says how many are behind it", () => {
    ...
    const card = screen.getByTestId("kapable-test-assertions-card");
