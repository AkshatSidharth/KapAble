...
describe("explore_chat_history streaming preview", () => {
  ...
  it("reproduces the pending card emitted while the history sub-agent is still running", async () => {
    ...
    render(
      <Provider store={store}>
        <ChatStreamProvider manager={manager}>
          <KapableMarkdownParser content="" showStreamingPreview />
        </ChatStreamProvider>
      </Provider>,
    );
    ...
    expect(latestPreviewXml).toContain("<kapable-explore-chat-history");
    expect(latestPreviewXml).not.toContain("</kapable-explore-chat-history>");

    const card = screen.getByTestId("kapable-explore-chat-history");
    ...
    expect(screen.queryByTestId("kapable-search-chats")).toBeNull();
  ...
  it("normalizes mixed read_chat arguments before the pending card completes", async () => {
    ...
    expect(previewXml).toBe(
      '<kapable-read-chat chat-id="703" state="pending">Reading chat...</kapable-read-chat>',
    );
    ...
    const card = screen.getByTestId("kapable-read-chat");
    ...
    expect(screen.queryByTestId("kapable-explore-chat-history")).toBeNull();
      ...
      view.rerender(
        <Provider store={store}>
          <ChatStreamProvider manager={manager}>
            <KapableMarkdownParser
              content={
                '<think>Considering the cited chat</think>\n<kapable-read-chat chat-id="703" title="Build indie shop landing page" range="4–7 of 9">Historical messages</kapable-read-chat>'
              }
              showStreamingPreview
            />
          </ChatStreamProvider>
    ...
    expect(screen.getAllByTestId("kapable-read-chat")).toHaveLength(1);
