...
export function TestsPanel() {
  ...
  // With the experiment enabled, "headed" means visible in KapAble's preview
  ...
  const switchToPublishableKey = useCallback(async () => {
    ...
    try {
      ...
      // warning. "not-applicable" means the key is still legacy and KapAble
  ...
  const startRecording = useCallback(() => {
    ...
    // Only a route the user picked through KapAble's chrome becomes the session's
    ...
    const startPath =
      previewIframeState.currentUrlSource === "kapable"
  ...
  const recordButtonTitle = !devServerRunning
    ...
    : isRunning
      ...
      : isRecordingSession
        ...
        : "Click through your app in the preview and KapAble writes the test for you.";
        ...
        <div className="flex items-start gap-2 max-w-sm mb-5 px-3 py-2 rounded-md bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 text-left text-[13px] text-amber-800 dark:text-amber-200">
          ...
          <span>
            {hasSupabaseIsolation
              ...
              : hasManagedDatabase
                ? "KapAble can't isolate this database in the current setup. These tests can create, update, or delete current data, so we strongly recommend enabling data backups before running them."
                : "These tests can create, update, or delete real data, and KapAble can't isolate a custom or non-database backend. We strongly recommend enabling data backups before running tests, in case they do something unintended."}
          </span>
        </div>
      )}
