...
describe("buildConfigForFramework", () => {
  ...
  it("leaves a start command alone for a framework KapAble never sets one for", () => {
    // Next.js carries `next start`, so KapAble has never supplied a command for
  ...
  describe.each([...APP_FRAMEWORK_TYPES, null])(
    ...
    (framework: AppFrameworkType | null) => {
      ...
      it("claims nothing beyond the build pack, the port, and a start command", () => {
        // Everything else is the build pack's to decide, and a field KapAble
