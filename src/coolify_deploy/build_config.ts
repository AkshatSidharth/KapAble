...
/**
 * The part of a build configuration KapAble supplies.
 *
 * One field, because there is one thing left worth saying. Anything KapAble adds
 * here is a claim that overrides what the build pack would have decided, and
 * is worth making only where KapAble knows something the build pack cannot read
 * off the app.
 */
interface FrameworkKnowledge {
  /**
   * Used only when the app declares no entry point of its own. An app that
   * names one is describing something KapAble cannot see — a changed Nitro
 ...
 * Coolify's separate port mappings, which KapAble does not set.
 */
const DEFAULT_PORT = "3000";

/**
 * What KapAble knows about each framework, over and above what the build pack
...
export function buildConfigForFramework(
  ...
  return {
    ...
    // Absent where KapAble has never supplied one, so a value the user set in
