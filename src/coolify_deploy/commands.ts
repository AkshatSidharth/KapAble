...
/**
 * Puts KapAble's public key on the repository as a deploy key.
 *
...
async function ensureGithubDeployKey({
  ...
  const res = await githubFetch(
    `${getGitHubApiBase()}/repos/${owner}/${repo}/keys`,
    {
      ...
      body: JSON.stringify({
        title: "KapAble deploy key (Coolify)",
        key: publicKey,
        read_only: true,
      }),
    },
  ...
  if (res.status === 422 && /already in use/i.test(body)) {
      ...
      try {
        ...
        throw new KapableError(
          `GitHub returned something other than a key list for ${owner}/${repo}, ` +
            `so KapAble could not tell whether the deploy key it holds is already ` +
    ...
    throw new KapableError(
      `The deploy key for ${owner}/${repo} is already registered on a different ` +
        ...
        `generate a new ` +
        `one — KapAble registers a regenerated key with Coolify under a new name.`,
  ...
  if (res.status === 401 || res.status === 403) {
    ...
    // for the user as having none, and rules/kapable-errors.md puts both under
...
async function warnIfBranchNotPushed({
  ...
  // at edits KapAble has just made has no way to tell they are not in this
  // build.
  try {
...
export async function runDeployPipeline({
  ...
  if (!app) {
    throw new KapableError(`App ${appId} not found`, KapableErrorKind.NotFound);
  }
  ...
  const resolvedAppPath = getKapableAppPath(app.path);
  ...
  const key = await client.registerPrivateKey({
    ...
    description: "Key KapAble uses to let Coolify clone this repository",
  ...
  const applicationUuid = await resolveApplication({
    ...
    create: async () => {
      const created = await client.createApplicationFromPrivateRepo({
        ...
        gitRepository,
        gitBranch,
        name: `kapable-${app.name}`.toLowerCase().replace(/[^a-z0-9-]/g, "-"),
        build,
  ...
  const database = await resolveDatabaseEnv(app, report).catch((error) => {
    ...
    // Precondition, an expired token is Auth — and rules/kapable-errors.md keeps
