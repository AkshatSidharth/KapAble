 ...
 * back to asking the user for a token, which is the path KapAble has always had.
 */
...
export async function readCoolifyVersion(
  ...
  try {
    ...
    throw new KapableError(
      "KapAble could not read which version of Coolify this is, so it could not " +
    ...
    if (error instanceof SshError && error.failure === "command-timeout") {
      ...
      throw new KapableError(
        "Coolify did not answer in time when KapAble asked which version it " +
...
/**
 * Creates an API token for KapAble.
 *
...
export async function mintApiToken(
  ...
  const output = await runTinker(
    ...
    // was meant to take.
    [
      `$u = \\App\\Models\\User::where('email', getenv('KAPABLE_ADMIN_EMAIL'))->first();`,
    ...
    {
      env: { KAPABLE_ADMIN_EMAIL: adminEmail },
      signal,
 ...
 * because that is not a failure — it is the ordinary case of an instance KapAble
