...
const REAL_TRANSCRIPT = [
  ...
  '> echo "__KAPABLE_OUT_END__" . PHP_EOL;',
...
const REAL_4_3_14 = [
  ...
  '> echo PHP_EOL . "__KAPABLE_OUT_END__" . PHP_EOL;',
  "> __KAPABLE_OUT_START__",
  "4.3.14",
  "__KAPABLE_OUT_END__",
].join("\n");
...
describe("finding our output however psysh prompts", () => {
  it("reads it when the prompt is missing or repeated", () => {
    ...
    for (const marker of [
      ...
      expect(
        extractOutput([marker, "answer", "__KAPABLE_OUT_END__"].join("\n")),
      ).toBe("answer");
    }
  });
...
describe("runTinker", () => {
  ...
  it("passes a secret by name and value, quoted", async () => {
    const session = fakeSession(() => ({ stdout: REAL_TRANSCRIPT }));
    await runTinker(session, "echo getenv('KAPABLE_SECRET');", {
...
describe("wrapScript", () => {
  it("breaks the line before the closing marker", () => {
    ...
    // `yes__KAPABLE_OUT_END__`, and the marker was never found.
    expect(wrapScript("echo 'yes';")).toContain(
      'echo PHP_EOL . "__KAPABLE_OUT_END__"',
    );
  });
  ...
  it("puts the body between the markers", () => {
    ...
    expect(lines[0]).toContain("__KAPABLE_OUT_START__");
    expect(lines[1]).toBe("echo 42;");
    expect(lines[2]).toContain("__KAPABLE_OUT_END__");
  });
});
