...
const mocks = vi.hoisted(() => ({
  ...
  streamMessage: vi.fn(),
  settings: {
    current: { enableKapablePro: true, agentToolConsents: {} },
  },
  quota: {
...
describe("buildPreCommitFixPrompt", () => {
  it("preserves the manual commit intent and asks the agent to verify fixes", () => {
    ...
    expect(prompt).toMatch(/KAPABLE_PRE_COMMIT_OUTPUT_.+_BEGIN/);
...
describe("useFixPreCommitWithAI", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mocks.settings.current = { enableKapablePro: true, agentToolConsents: {} };
    mocks.quota.current = {
  ...
  it("names an exhausted quota as the reason so the dialog can say so", () => {
    mocks.settings.current = { enableKapablePro: false, agentToolConsents: {} };
    mocks.quota.current = {
      isQuotaExceeded: true,
      isLoading: false,
      error: null,
