...
import { type UserSettings, hasKapableProKey } from "@/lib/schemas";
import {
...
const TELEMETRY_CONSENT_KEY = "kapableTelemetryConsent";
const TELEMETRY_USER_ID_KEY = "kapableTelemetryUserId";
const KAPABLE_PRO_STATUS_KEY = "kapableProStatus";
...
export function isKapableProUser(): boolean {
  return window.localStorage.getItem(KAPABLE_PRO_STATUS_KEY) === "true";
}
...
function processSettingsForTelemetry(settings: UserSettings) {
  ...
  window.localStorage.setItem(
    KAPABLE_PRO_STATUS_KEY,
    hasKapableProKey(settings) ? "true" : "false",
  );
}
