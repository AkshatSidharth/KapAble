  ...
  const contentWindow = {
    postMessage: (message: unknown, targetOrigin?: string) => {
      ...
      if (
        ...
        message !== null &&
        "type" in message &&
        message.type === "flush-kapable-recorder" &&
        "requestId" in message
      ) {
        queueMicrotask(() =>
          send({
            type: "kapable-recorder-flushed",
...
function reloadAnnouncing(iframe?: ReturnType<typeof makeIframe>) {
  return () => iframe?.send({ type: "kapable-recorder-initialized" });
}
...
describe("useTestRecorder", () => {
  ...
  // A route the user picked through KapAble's chrome IS the starting point, and it
  ...
  it("flushes an iframe action already queued when review is requested", async () => {
    ...
    const flush = iframe.posted.find(
      (message) => message.type === "flush-kapable-recorder",
    );
  ...
  it("waits for the reloaded preview before it says it is recording", async () => {
    ...
    expect(iframe.posted).not.toContainEqual({
      type: "activate-kapable-recorder",
    });

    await act(async () => {
      iframe.send({ type: "kapable-recorder-initialized" });
      await started;
    });
  ...
  it("ignores navigations the app makes on its own", async () => {
    ...
    // The shim (worker/kapable-shim.js) reports every history change the app makes,
  ...
  it("records a navigation made from KapAble's own address bar", async () => {
    const { result } = await recordingSession({
      iframe: makeIframe(),
      appUrl: true,
    });
  ...
  it("hands the session back when the preview unmounts mid-recording", async () => {
    ...
    expect(iframe.posted).toContainEqual({
      type: "deactivate-kapable-recorder",
      token: AUTH_BOOTSTRAP_TOKEN,
    });
  });
  ...
  it("withholds credentials until the preview's origin is known", async () => {
    ...
    act(() => {
      iframe.send({ type: "kapable-auth-bootstrap-ready" });
    });
  ...
  it("refuses the review when teardown reported the environment broken", async () => {
    ...
    stopRecordingMock.mockImplementationOnce(async () => {
      onEnded({
        appId: 1,
        reason: "error",
        message: "KapAble couldn't restore your app's real database settings",
      });
      return { ok: true };
    });
