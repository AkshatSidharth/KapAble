 ...
 * isn't listening costs nothing — the re-arm on `kapable-recorder-initialized`
...
export function useTestRecorder({
  ...
  >(null);
  // Settled by the `kapable-recorder-initialized` of the document the recording is
  ...
  /**
   * Record a `page.goto` for a navigation the user made in KapAble's own chrome:
  ...
  const postRecorderControl = useCallback(
    (targetAppId: number, type: "activate" | "deactivate") => {
      ...
      postToIframe({ type: `${type}-kapable-recorder`, token }, origin);
    },
  ...
  useEffect(() => {
    const handler = (e: MessageEvent) => {
      ...
      switch (data.type) {
        case "kapable-recorder-action": {
          record(data.action);
          break;
        }
        case "kapable-recorder-initialized": {
        ...
        case "kapable-auth-bootstrap-ready": {
  ...
  // mid-load; this effect plus the re-arm on `kapable-recorder-initialized` make
  ...
  // the bootstrap's `kapable-auth-bootstrap-ready` announce is no rescue: the
  ...
  const beginRecording = useCallback(
    async (
      ...
      if (auth.mode !== "none") {
        ...
        // could announce — the re-arm on `kapable-recorder-initialized` picks up
