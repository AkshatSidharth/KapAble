...
describe("local-agent grep cards (integration)", () => {
  ...
  it("renders grep cards and expands their results", async () => {
    ...
    await waitFor(
      () => expect(screen.getAllByTestId("kapable-grep")).toHaveLength(2),
      { timeout: 20_000 },
    );
    const cards = screen.getAllByTestId("kapable-grep");
    ...
    expect(assistantContent).toContain("<kapable-grep");
  ...
  it("can include ignored files when the agent requests them", async () => {
    ...
    const grepCard = await screen.findByTestId("kapable-grep", undefined, {
      timeout: 20_000,
    });
    await waitFor(() =>
