...
// against the checked-out fixture app, and the resulting <kapable-list-files>
...
// renders the same KapableListFiles cards in the DOM (asserted below via
// data-testid="kapable-list-files"), and the same listing is still asserted
...
describe("local-agent list_files (integration)", () => {
  ...
  it("lists files non-recursively then recursively", async () => {
    ...
    await waitFor(
      () => expect(screen.getByTestId("kapable-list-files")).toBeTruthy(),
      { timeout: 20_000 },
    );
    await waitFor(
      () =>
  ...
  it("lists ignored files with include_ignored", async () => {
    // The e2e used the minimal-with-kapable fixture app (it has a git-ignored
    // .kapable/plans/test-plan.md). Check it out as a second app in this
    ...
    const fixtureAppDir = path.join(
      ...
      "fixtures",
      "import-app",
      "minimal-with-kapable",
    );
    const appDir = path.join(path.dirname(harness.appDir), "app-with-kapable");
    ...
    const [appRow] = await harness.db
      .insert(apps)
      .values({ name: "minimal-with-kapable", path: appDir })
      .returning();
    ...
    // The list_files tool card renders in the DOM with the ignored .kapable file.
    await waitFor(
      () => expect(screen.getByTestId("kapable-list-files")).toBeTruthy(),
      { timeout: 20_000 },
    );
    await waitFor(
      () =>
        expect(
          screen.getByText(/Here are the ignored \.kapable files\./),
        ).toBeTruthy(),
      { timeout: 20_000 },
    );
    ...
    expect(assistant.content).toContain(
      'directory=".kapable" recursive="true" include_ignored="true"',
    );
    expect(assistant.content).toContain(".kapable/plans/test-plan.md");
    expect(assistant.content).toContain("Here are the ignored .kapable files.");
