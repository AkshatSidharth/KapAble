...
// (`getByTestId("kapable-search-replace")` visible), which the hybrid harness lets
...
describe("local agent search_replace (hybrid)", () => {
  ...
  it("applies a targeted search_replace edit and renders the tool card", async () => {
    ...
    // getByTestId("kapable-search-replace").
    await waitFor(
      () => expect(screen.getByTestId("kapable-search-replace")).toBeTruthy(),
      { timeout: 20_000 },
    );
...
export default App;`,
    ...
    expect(assistant.content).toContain("kapable-search-replace");
