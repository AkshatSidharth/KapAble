...
// the turn, streaming `<kapable-status>` progress updates
...
// renders as the real <kapable-status> card in the messages list (the surface
...
describe("local agent supabase deploy progress (integration)", () => {
  ...
  it("shows Supabase deploy queue progress and completes 20/20", async () => {
    ...
    // The finished <kapable-status> card renders in the messages list — the same
    ...
    expect(assistant.content).toContain(
      '<kapable-status title="Supabase functions deployed: 20/20 complete" state="finished">',
    );
