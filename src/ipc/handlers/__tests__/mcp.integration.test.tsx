...
describe("MCP chat flows (integration)", () => {
  ...
  it("renders MCP tool search cards in local-agent search mode", async () => {
    const previousThreshold = process.env.KAPABLE_MCP_INLINE_TOKEN_THRESHOLD;
    process.env.KAPABLE_MCP_INLINE_TOKEN_THRESHOLD = "0";
    try {
      await harness.mcp.addStdioServer();
      ...
      if (previousThreshold === undefined) {
        delete process.env.KAPABLE_MCP_INLINE_TOKEN_THRESHOLD;
      } else {
        process.env.KAPABLE_MCP_INLINE_TOKEN_THRESHOLD = previousThreshold;
      }
    }
  }, 60_000);
