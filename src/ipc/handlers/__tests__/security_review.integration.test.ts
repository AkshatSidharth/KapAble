...
//    <kapable-security-finding> tags, which render in the messages list (the
...
describe("security review (integration)", () => {
  ...
  it("runs a security review and parses findings", async () => {
    ...
    // <kapable-security-finding> cards' content (the same DOM surface the e2e
    // asserted on).
    await waitFor(
      () =>
        expect(
    ...
    expect(assistant.content).toContain("<kapable-security-finding");
...
${finding.description}`;
  ...
  it("edit and use knowledge: SECURITY_RULES.md is added to the review context", async () => {
    ...
    expect(dump.text).not.toContain('<kapable-file path="SECURITY_RULES.md">');
    const raw = JSON.parse(fs.readFileSync(dump.dumpPath, "utf-8"));
    const systemMessage = raw.body.messages.find(
      (m: { role: string }) => m.role === "system",
    );
