...
} from "@/ipc/utils/kapable_tag_parser";
...
// Mock paths module to control getKapableAppPath
vi.mock("@/paths/paths", () => ({
  getKapableAppPath: vi.fn().mockImplementation((appPath) => {
...
describe("processStreamChunks", () => {
  it("replaces partial output with an inline warning for Fable refusals", async () => {
    ...
    expect(result.incrementalResponse).toContain("<kapable-output");
...
describe("getKapableAddDependencyTags", () => {
  ...
  it("should return an array of kapable-add-dependency tags", () => {
    const result = getKapableAddDependencyTags(
      `<kapable-add-dependency packages="uuid"></kapable-add-dependency>`,
    );
    expect(result).toEqual(["uuid"]);
  });

  it("should return all the packages in the kapable-add-dependency tags", () => {
    const result = getKapableAddDependencyTags(
      `<kapable-add-dependency packages="pkg1 pkg2"></kapable-add-dependency>`,
    );
  ...
  it("should return all the packages in the kapable-add-dependency tags", () => {
    const result = getKapableAddDependencyTags(
      `txt before<kapable-add-dependency packages="pkg1 pkg2"></kapable-add-dependency>text after`,
    );
    expect(result).toEqual(["pkg1", "pkg2"]);
  });

  it("should return all the packages in multiple kapable-add-dependency tags", () => {
    const result = getKapableAddDependencyTags(
      `txt before<kapable-add-dependency packages="pkg1 pkg2"></kapable-add-dependency>txt between<kapable-add-dependency packages="pkg3"></kapable-add-dependency>text after`,
    );
  ...
  it("preserves scoped version specs and ignores extra whitespace", () => {
    const result = getKapableAddDependencyTags(
      `<kapable-add-dependency packages="  foo@latest   @scope/bar@^2.0.0 "></kapable-add-dependency>`,
    );
    expect(result).toEqual(["foo@latest", "@scope/bar@^2.0.0"]);
  });
});
describe("getKapableWriteTags", () => {
  it("should return an empty array when no kapable-write tags are found", () => {
    const result = getKapableWriteTags("No kapable-write tags here");
    expect(result).toEqual([]);
  });

  it("should return a kapable-write tag", () => {
    const result =
      getKapableWriteTags(`<kapable-write path="src/components/TodoItem.tsx" description="Creating a component for individual todo items">
...
console.log("TodoItem");`,
  ...
  it("should strip out code fence (if needed) from a kapable-write tag", () => {
    const result =
      getKapableWriteTags(`<kapable-write path="src/components/TodoItem.tsx" description="Creating a component for individual todo items">
\`\`\`tsx
import React from "react";
...
console.log("TodoItem");`,
  ...
  it("should handle missing description", () => {
    const result = getKapableWriteTags(`
      <kapable-write path="src/pages/locations/neighborhoods/louisville/Highlands.tsx">
import React from 'react';
</kapable-write>
    `);
    expect(result).toEqual([
...
AFTER TAG
    ...
    const result = getKapableWriteTags(cleanedInput);
    expect(result).toEqual([
      {
        path: "src/pages/locations/neighborhoods/louisville/Highlands.tsx",
        description: "Updating Highlands neighborhood page to use ＜a＞ tags.",
  ...
  it("should handle multiple nested tags after preprocessing", () => {
    const inputWithMultipleNestedTags = `<kapable-write path="src/file.tsx" description="Testing <div> and <span> and <a> tags.">content</kapable-write>`;
  ...
  it("should handle nested tags in multiple attributes", () => {
    const inputWithNestedInMultipleAttrs = `<kapable-write path="src/<component>.tsx" description="Testing <div> tags.">content</kapable-write>`;
  ...
  it("should return an array of kapable-write tags", () => {
    const result = getKapableWriteTags(
...
<kapable-write path="src/types/todo.ts" description="Creating types for our todo items">
...
<kapable-write path="src/components/TodoItem.tsx" description="Creating a component for individual todo items">
import React from "react";
...
<kapable-write path="src/components/TodoForm.tsx" description="Creating a form component for adding new todos">
...
<kapable-write path="src/components/TodoList.tsx" description="Creating the main todo list component">
import React from "react";
import { Todo } from "../types/todo";
...
<kapable-write path="src/components/TodoStats.tsx" description="Creating a component to show todo statistics">
import React from "react";
import { Todo } from "../types/todo";
...
<kapable-write path="src/pages/Index.tsx" description="Creating the main page for our todo app">
...
<kapable-write path="src/types/uuid.d.ts" description="Adding type definitions for uuid">
declare module 'uuid' {
...
describe("getKapableRenameTags", () => {
  it("should return an empty array when no kapable-rename tags are found", () => {
    const result = getKapableRenameTags("No kapable-rename tags here");
    expect(result).toEqual([]);
  });

  it("should return an array of kapable-rename tags", () => {
    const result = getKapableRenameTags(
      `<kapable-rename from="src/components/UserProfile.jsx" to="src/components/ProfileCard.jsx"></kapable-rename>
      <kapable-rename from="src/utils/helpers.js" to="src/utils/utils.js"></kapable-rename>`,
    );
    expect(result).toEqual([
      {
...
describe("getKapableDeleteTags", () => {
  it("should return an empty array when no kapable-delete tags are found", () => {
    const result = getKapableDeleteTags("No kapable-delete tags here");
    expect(result).toEqual([]);
  });

  it("should return an array of kapable-delete paths", () => {
    const result = getKapableDeleteTags(
      `<kapable-delete path="src/components/Analytics.jsx"></kapable-delete>
      <kapable-delete path="src/utils/unused.js"></kapable-delete>`,
    );
    expect(result).toEqual([
...
describe("processFullResponse", () => {
  ...
  it("should return empty object when no kapable-write tags are found", async () => {
    const result = await processFullResponseActions(
      "No kapable-write tags here",
      1,
      {
  ...
  it("should process kapable-write tags and create files", async () => {
    // Set up fs mocks to succeed
    vi.mocked(fs.mkdirSync).mockImplementation(() => undefined);
    vi.mocked(fs.writeFileSync).mockImplementation(() => undefined);

    const response = `<kapable-write path="src/file1.js">console.log('Hello');</kapable-write>`;
  ...
  it("should process multiple kapable-write tags and commit all files", async () => {
    ...
    const response = `
    <kapable-write path="src/file1.js">console.log('First file');</kapable-write>
    <kapable-write path="src/utils/file2.js">export const add = (a, b) => a + b;</kapable-write>
    <kapable-write path="src/components/Button.tsx">
    import React from 'react';
  ...
  it("should process kapable-rename tags and rename files", async () => {
    // Set up fs mocks to succeed
    vi.mocked(fs.existsSync).mockReturnValue(true);
    vi.mocked(fs.mkdirSync).mockImplementation(() => undefined);
    vi.mocked(fs.renameSync).mockImplementation(() => undefined);

    const response = `<kapable-rename from="src/components/OldComponent.jsx" to="src/components/NewComponent.jsx"></kapable-rename>`;
  ...
  it("should handle non-existent files during rename gracefully", async () => {
    ...
    const response = `<kapable-rename from="src/components/NonExistent.jsx" to="src/components/NewFile.jsx"></kapable-rename>`;
  ...
  it("should process kapable-delete tags and delete files", async () => {
    // Set up fs mocks to succeed
    vi.mocked(fs.existsSync).mockReturnValue(true);
    vi.mocked(fs.unlinkSync).mockImplementation(() => undefined);

    const response = `<kapable-delete path="src/components/Unused.jsx"></kapable-delete>`;
  ...
  it.each([
    ...
    async (deletePath) => {
      const response = `<kapable-delete path="${deletePath}"></kapable-delete>`;
  ...
  it("should preflight all deletes before applying any of them", async () => {
    ...
    const response = `
      <kapable-write path="src/new.jsx">export default {};</kapable-write>
      <kapable-rename from="src/old.jsx" to="src/renamed.jsx"></kapable-rename>
      <kapable-delete path="src/keep.jsx"></kapable-delete>
      <kapable-delete path="foo/.."></kapable-delete>
    `;
  ...
  it.each(["../sibling", "..\\sibling"])(
    ...
    async (deletePath) => {
      const result = await processFullResponseActions(
        `<kapable-delete path="${deletePath}"></kapable-delete>`,
        1,
  ...
  it("unlinks a slash-terminated final symlink instead of following it", async () => {
    ...
    const result = await processFullResponseActions(
      `<kapable-delete path="self/"></kapable-delete>`,
      1,
      { chatSummary: undefined, messageId: 1 },
    );
  ...
  it("should handle non-existent files during delete gracefully", async () => {
    ...
    const response = `<kapable-delete path="src/components/NonExistent.jsx"></kapable-delete>`;
  ...
  it("should process mixed operations (write, rename, delete) in one response", async () => {
    ...
    const response = `
    <kapable-write path="src/components/NewComponent.jsx">import React from 'react'; export default () => <div>New</div>;</kapable-write>
    <kapable-rename from="src/components/OldComponent.jsx" to="src/components/RenamedComponent.jsx"></kapable-rename>
    <kapable-delete path="src/components/Unused.jsx"></kapable-delete>
    `;
...
describe("removeKapableTags", () => {
  it("should return empty string when input is empty", () => {
    const result = removeKapableTags("");
    expect(result).toBe("");
  });

  it("should return the same text when no kapable tags are present", () => {
    const text = "This is a regular text without any kapable tags.";
    const result = removeKapableTags(text);
    expect(result).toBe(text);
  });

  it("should remove a single kapable-write tag", () => {
    const text = `Before text <kapable-write path="src/file.js">console.log('hello');</kapable-write> After text`;
    const result = removeKapableTags(text);
    expect(result).toBe("Before text  After text");
  });

  it("should remove a single kapable-delete tag", () => {
    const text = `Before text <kapable-delete path="src/file.js"></kapable-delete> After text`;
    const result = removeKapableTags(text);
    expect(result).toBe("Before text  After text");
  });

  it("should remove a single kapable-rename tag", () => {
    const text = `Before text <kapable-rename from="old.js" to="new.js"></kapable-rename> After text`;
    const result = removeKapableTags(text);
    expect(result).toBe("Before text  After text");
  });

  it("should remove multiple different kapable tags", () => {
    const text = `Start <kapable-write path="file1.js">code here</kapable-write> middle <kapable-delete path="file2.js"></kapable-delete> end <kapable-rename from="old.js" to="new.js"></kapable-rename> finish`;
    const result = removeKapableTags(text);
    expect(result).toBe("Start  middle  end  finish");
  });

  it("should remove kapable tags with multiline content", () => {
    const text = `Before
<kapable-write path="src/component.tsx" description="A React component">
import React from 'react';
...
After`;
  ...
  it("should handle kapable tags with complex attributes", () => {
    const text = `Text <kapable-write path="src/file.js" description="Complex component with quotes" version="1.0">const x = "hello world";</kapable-write> more text`;
    const result = removeKapableTags(text);
    expect(result).toBe("Text  more text");
  });

  it("should remove kapable tags and trim whitespace", () => {
    const text = `  <kapable-write path="file.js">code</kapable-write>  `;
    const result = removeKapableTags(text);
    expect(result).toBe("");
  });

  it("should handle nested content that looks like tags", () => {
    const text = `<kapable-write path="file.js">
...
const component = <Component />;
  ...
  it("should handle self-closing kapable tags", () => {
    const text = `Before <kapable-delete path="file.js" /> After`;
    const result = removeKapableTags(text);
    expect(result).toBe('Before <kapable-delete path="file.js" /> After');
  });
...
const special = "Special chars: @#$%^&*()[]{}|\\";
  ...
  it("should handle multiple kapable tags of the same type", () => {
    const text = `<kapable-write path="file1.js">code1</kapable-write> between <kapable-write path="file2.js">code2</kapable-write>`;
    const result = removeKapableTags(text);
    expect(result).toBe("between");
  });

  it("should handle kapable tags with custom tag names", () => {
    const text = `Before <kapable-custom-action param="value">content</kapable-custom-action> After`;
    const result = removeKapableTags(text);
    expect(result).toBe("Before  After");
  });
});

describe("hasUnclosedKapableWrite", () => {
  it("should return false when there are no kapable-write tags", () => {
    const text = "This is just regular text without any kapable tags.";
    const result = hasUnclosedKapableWrite(text);
    expect(result).toBe(false);
  });

  it("should return false when kapable-write tag is properly closed", () => {
    const text = `<kapable-write path="src/file.js">console.log('hello');</kapable-write>`;
    const result = hasUnclosedKapableWrite(text);
    expect(result).toBe(false);
  });

  it("should return true when kapable-write tag is not closed", () => {
    const text = `<kapable-write path="src/file.js">console.log('hello');`;
    const result = hasUnclosedKapableWrite(text);
    expect(result).toBe(true);
  });

  it("should return false when kapable-write tag with attributes is properly closed", () => {
    const text = `<kapable-write path="src/file.js" description="A test file">console.log('hello');</kapable-write>`;
    const result = hasUnclosedKapableWrite(text);
    expect(result).toBe(false);
  });

  it("should return true when kapable-write tag with attributes is not closed", () => {
    const text = `<kapable-write path="src/file.js" description="A test file">console.log('hello');`;
    const result = hasUnclosedKapableWrite(text);
    expect(result).toBe(true);
  });

  it("should return false when there are multiple closed kapable-write tags", () => {
    const text = `<kapable-write path="src/file1.js">code1</kapable-write>
    Some text in between
    <kapable-write path="src/file2.js">code2</kapable-write>`;
    const result = hasUnclosedKapableWrite(text);
    expect(result).toBe(false);
  });

  it("should return true when the last kapable-write tag is unclosed", () => {
    const text = `<kapable-write path="src/file1.js">code1</kapable-write>
    Some text in between
    <kapable-write path="src/file2.js">code2`;
    const result = hasUnclosedKapableWrite(text);
    expect(result).toBe(true);
  });

  it("should return false when first tag is unclosed but last tag is closed", () => {
    const text = `<kapable-write path="src/file1.js">code1
    Some text in between
    <kapable-write path="src/file2.js">code2</kapable-write>`;
    const result = hasUnclosedKapableWrite(text);
    expect(result).toBe(false);
  ...
  it("should handle multiline content correctly", () => {
    const text = `<kapable-write path="src/component.tsx" description="React component">
import React from 'react';
...
export default Component;`;
  ...
  it("should handle complex attributes correctly", () => {
    const text = `<kapable-write path="src/file.js" description="File with quotes and special chars" version="1.0" author="test">
...
const regex = /<div[^>]*>/g;
  ...
  it("should handle text before and after kapable-write tags", () => {
    const text = `Some text before the tag
<kapable-write path="src/file.js">console.log('hello');</kapable-write>
Some text after the tag`;
  ...
  it("should handle unclosed tag with text after", () => {
    const text = `Some text before the tag
<kapable-write path="src/file.js">console.log('hello');
Some text after the unclosed tag`;
    const result = hasUnclosedKapableWrite(text);
    expect(result).toBe(true);
  });

  it("should handle empty kapable-write tags", () => {
    const text = `<kapable-write path="src/file.js"></kapable-write>`;
    const result = hasUnclosedKapableWrite(text);
    expect(result).toBe(false);
  });

  it("should handle unclosed empty kapable-write tags", () => {
    const text = `<kapable-write path="src/file.js">`;
    const result = hasUnclosedKapableWrite(text);
    expect(result).toBe(true);
  });

  it("should focus on the last opening tag when there are mixed states", () => {
    const text = `<kapable-write path="src/file1.js">completed content</kapable-write>
    <kapable-write path="src/file2.js">unclosed content
    <kapable-write path="src/file3.js">final content</kapable-write>`;
    const result = hasUnclosedKapableWrite(text);
    expect(result).toBe(false);
  });

  it("should handle tags with special characters in attributes", () => {
    const text = `<kapable-write path="src/file-name_with.special@chars.js" description="File with special chars in path">content</kapable-write>`;
    const result = hasUnclosedKapableWrite(text);
    expect(result).toBe(false);
  });
});
