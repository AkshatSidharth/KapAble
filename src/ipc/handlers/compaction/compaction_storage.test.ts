...
<kapable-mcp-tool-call server="filesystem" tool="read_file">
{"path": "/src/index.ts"}
    ...
    expect(result).not.toContain("kapable-mcp-tool-call");
  });

  it("transforms tool-call tags that carry extra attributes", () => {
    const content = `<kapable-mcp-tool-call server="filesystem" tool="read_file" auto-approved-reason="Reads a project file.">
{"path": "/src/index.ts"}
</kapable-mcp-tool-call>`;
  ...
  it("transforms tool-result tags and includes char count", () => {
    const content = `<kapable-mcp-tool-result server="filesystem" tool="read_file">
short result
  ...
  it("transforms tags that carry a call-id attribute", () => {
    const content = `<kapable-mcp-tool-call server="filesystem" tool="read_file" call-id="abc-123">
{"path": "/a.ts"}
</kapable-mcp-tool-call>
<kapable-mcp-tool-result server="filesystem" tool="read_file" call-id="abc-123">
ok
</kapable-mcp-tool-result>`;
    ...
    expect(result).not.toContain("kapable-mcp-tool-call");
    expect(result).not.toContain("kapable-mcp-tool-result");
...
${longContent}
  ...
  it("does not truncate results at exactly the limit", () => {
    ...
    const content = `<kapable-mcp-tool-result server="fs" tool="read">
${exactContent}
</kapable-mcp-tool-result>`;
...
<kapable-mcp-tool-call server="fs" tool="read_file">
{"path": "/a.ts"}
</kapable-mcp-tool-call>
<kapable-mcp-tool-result server="fs" tool="read_file">
contents of a
</kapable-mcp-tool-result>
<kapable-mcp-tool-call server="fs" tool="read_file">
{"path": "/b.ts"}
...
contents of b
    ...
    expect(result).not.toContain("kapable-mcp");
  });
...
<kapable-mcp-tool-call server="fs" tool="write_file">
{"path": "/a.ts", "content": "new"}
</kapable-mcp-tool-call>
<kapable-mcp-tool-result server="fs" tool="write_file">
success
</kapable-mcp-tool-result>`;
...
describe("formatAsTranscript", () => {
  ...
  it("handles multi-turn conversation with interleaved tool use", () => {
    ...
    // No original kapable tags remain
    expect(result).not.toContain("kapable-mcp");
  ...
  it("handles assistant messages mixing multiple servers", () => {
    const messages: CompactionMessage[] = [
      { role: "user", content: "Search and then write a file" },
      {
        ...
        content: [
          "I'll search first.",
          '<kapable-mcp-tool-call server="search" tool="grep">',
          '{"pattern": "TODO"}',
          "</kapable-mcp-tool-call>",
          '<kapable-mcp-tool-result server="search" tool="grep">',
          "src/a.ts:3: // TODO fix",
          "</kapable-mcp-tool-result>",
          "Found one. Now writing the fix.",
          '<kapable-mcp-tool-call server="filesystem" tool="write_file">',
          '{"path": "/src/a.ts", "content": "fixed"}',
          "</kapable-mcp-tool-call>",
          '<kapable-mcp-tool-result server="filesystem" tool="write_file">',
          "ok",
          "</kapable-mcp-tool-result>",
        ].join("\n"),
      },
