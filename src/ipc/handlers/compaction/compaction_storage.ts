...
/**
 * Get the backup directory for a specific chat within the app's .kapable/chats/ directory.
 */
function getChatBackupDir(appPath: string, chatId: number): string {
  return path.join(appPath, ".kapable", "chats", String(chatId));
}

/**
 * Transform kapable-specific tool XML tags to shorter, LLM-friendly equivalents
 * and truncate large tool results for token efficiency.
 */
export function transformToolTags(content: string): string {
  // Transform <kapable-mcp-tool-call> to <tool-use>. Tolerates extra attributes
  ...
  // Transform <kapable-mcp-tool-result> to <tool-result> with truncation
...
export async function storePreCompactionMessages(
  ...
  // Ensure directory exists and .kapable is gitignored
