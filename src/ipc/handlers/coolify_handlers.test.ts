...
describe("naming the stored token", () => {
  ...
  it("names a server KapAble set up, so the panel can hold the address to it", async () => {
    settings.coolify = {
  ...
  it("has no server address when KapAble set nothing up", async () => {
    const status: any = await call("coolify:get-status", { appId: 1 });
...
describe("clearing the token", () => {
  ...
  it("forgets every stored detail of the instance", async () => {
    // KapAble holds one Coolify at a time, so anything surviving here belongs to
  ...
  it("keeps everything when the same instance answers at a new address", async () => {
    ...
    // abandons applications that are still running, with nothing left in KapAble
    // that can reach them.
...
describe("the admin account KapAble created", () => {
  beforeEach(() => {
    settings.coolify = {
      ...(settings.coolify as Record<string, unknown>),
      admin: {
  ...
  it("survives the token for it being saved", async () => {
    ...
    // would drop the password on the way in, and KapAble has the only copy.
    await call("coolify:save-token", {
      instanceUrl: "https://coolify.example.com",
      token: "tok-2",
      acknowledgedInsecure: false,
