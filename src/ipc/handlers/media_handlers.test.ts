...
vi.mock("electron", () => ({
  ...
  app: {
    getPath: vi.fn(() => path.join(os.tmpdir(), "kapable-media-user-data")),
...
describe("media move recording admission", () => {
  ...
  beforeEach(() => {
    ...
    tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "kapable-media-handler-"));
  ...
  it("allows a move out of a recording source when the target is free", async () => {
    ...
    const sourceMediaPath = path.join(sourcePath, KAPABLE_MEDIA_DIR_NAME);
    ...
    expect(
      fs.existsSync(path.join(targetPath, KAPABLE_MEDIA_DIR_NAME, "image.png")),
    ).toBe(true);
  });
});
