      ...
      const result = await db
        ...
        .where(
          and(
            ...
            like(messages.content, "%<kapable-security-finding%"),
          ),
        )
...
function parseSecurityFindings(content: string): SecurityFinding[] {
  ...
  // Regex to match kapable-security-finding tags
  // Using lazy quantifier with proper boundaries to prevent catastrophic backtracking
  const regex =
    /<kapable-security-finding\s+title="([^"]+)"\s+level="(critical|high|medium|low)">([\s\S]*?)<\/kapable-security-finding>/g;
