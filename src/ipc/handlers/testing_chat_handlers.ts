...
${Array.from(
  { length: 5000 },
  (_, i) =>
    `<kapable-write path="src/stress/file_${i}.ts" description="stress file ${i}">
export const id${i} = ${i};
...
${Array.from({ length: 10000 }, (_, i) => {
  ...
  return `<kapable-write path="src/stress/file_${i}.ts" description="stress file ${i}">
export const id${i} = ${i};
export const name${i} = "file_${i}";
...
// explicit [kapable-qa=stress-...] prompt fires, not at module import.
const TEST_RESPONSES: Record<string, ResponseEntry> = {
  ...
  <kapable-add-dependency packages="deno"></kapable-add-dependency>

  EOM`,
  "add-non-existing-dep": `I'll add that dependency for you.

  <kapable-add-dependency packages="@angular/does-not-exist"></kapable-add-dependency>

  EOM`,
  "add-multiple-deps": `I'll add that dependency for you.

  <kapable-add-dependency packages="react-router-dom react-query"></kapable-add-dependency>

  EOM`,
  write: `Hello world
  <kapable-write path="src/hello.ts" content="Hello world">
...
export function getTestResponse(prompt: string): string | null {
  const match = prompt.match(/\[kapable-qa=([^\]]+)\]/);
  if (match) {
 ...
 * is local to fully-formed `<kapable-*>` tags and idempotent, so the cleaned
