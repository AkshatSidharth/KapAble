...
function formatDeniedBuildsNote(packageNames: string[]): string {
  ...
  return `\n\nNote: build scripts for ${packageList} were not run (KapAble security policy).`;
}
...
export async function executeAddDependency({
  ...
  const updatedContent = message.content.replace(
    new RegExp(
      `<kapable-add-dependency packages="(?:${buildPackagesAttrPattern(packages)})">[\\s\\S]*?</kapable-add-dependency>`,
      "g",
    ),
    `<kapable-add-dependency packages="${escapedPackages}">${escapeXmlContent(installResults)}</kapable-add-dependency>`,
  );
