...
vi.mock("@/paths/paths", async (importOriginal) => {
  ...
  return {
    ...
    getKapableAppPath: (appPath: string) =>
      nodePath.join(nodeOs.tmpdir(), "kapable-image-generation-tests", appPath),
  };
});
...
describe("ImageGenerationService", () => {
  const tempBase = path.join(os.tmpdir(), "kapable-image-generation-tests");
  ...
  it("serializes with app moves and saves using the refreshed app path", async () => {
    ...
    await expect(generation).resolves.toMatchObject({
      appPath: "moved-app",
      appName: "Moved app",
      filePath: expect.stringContaining(path.join("moved-app", ".kapable")),
    });
    await expect(
    ...
    ).resolves.toContain(".kapable/");
    expect(
      fs.existsSync(path.join(tempBase, "test-app", ".kapable", "media")),
    ).toBe(false);
  });
  ...
  it("waits for repository writers before committing a generated image", async () => {
    ...
    await expect(generation).resolves.toMatchObject({
      appPath: "test-app",
      filePath: expect.stringContaining(path.join("test-app", ".kapable")),
    });
  });
