...
function reconcileRestore(
  ...
  return {
    ...
    error: {
      message:
        "KapAble restarted during an interrupted version restore. Inspect and repair the repository before continuing.",
    },
      ...
      state =
        !ownsCheckout || reachedSafeBranch
          ...
          : {
              ...
              error: {
                message:
                  "KapAble restarted while switching branches. Return to the original branch before continuing.",
              },
            };
    } else if (
      current.type === "restoring" ||
      ...
      state =
        event.branch === current.session.originBranch
          ...
          : {
              ...
              error: {
                message:
                  "KapAble restarted during a version checkout. Return to the original branch before continuing.",
              },
            };
    }
    return {
