...
export const CoolifyStatusSchema = z.object({
  ...
  /** The Coolify KapAble is connected to, or null once it has been forgotten. */
  instanceUrl: z.string().nullable(),
  /**
   * The address of a server KapAble set up and holds an admin account for, or
   * null. Not a secret, and the panel needs it without the password: while
   * this is set the only Coolify KapAble will connect to is this one, so it
