...
export const SetupTargetSchema = SetupServerSchema.extend({
   ...
   *
   * Optional because KapAble can derive one from the address. Supplying one is
...
/**
 * What KapAble found when it looked at the server, before touching it.
 *
...
export const SetupResultSchema = z.object({
   ...
   *
   * KapAble asks for a certificate and settles for plain HTTP when none arrives,
  ...
  /**
   * Whether KapAble ended up holding a token, which is not the same as one
   ...
   *
   * Separate from tokenStored because KapAble enables the API first and mints
...
/**
 * What KapAble can tell the user about getting into their own server.
 *
 * Two records, each carrying its own address, because they are facts about
 * two different things: the instance KapAble talks to, and a machine KapAble built.
 ...
 *
 * Null where KapAble never had it: an instance connected by pasting a token has
 * no admin account KapAble created, and a server set up but not connected to has
 * no instance.
 */
...
export const coolifySetupContracts = {
  ...
  /**
   * The credentials for a server KapAble set up, on request.
   *
   ...
   *
   * DO NOT LOG this handler. A finished run carries the admin password KapAble
   ...
   * leaves KapAble unconnected rather than connected to something nobody agreed
