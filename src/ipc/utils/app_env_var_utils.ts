...
export function getEnvFilePath({ appPath }: { appPath: string }): string {
  return path.join(getKapableAppPath(appPath), ENV_FILE_NAME);
}
...
export async function updateDbPushEnvVar({
  ...
  try {
    ...
    // Update or add KAPABLE_DISABLE_DB_PUSH
    const existingVar = envVars.find(
      (envVar) => envVar.key === "KAPABLE_DISABLE_DB_PUSH",
    );
    if (existingVar) {
      existingVar.value = disabled ? "true" : "false";
    } else {
      envVars.push({
        key: "KAPABLE_DISABLE_DB_PUSH",
