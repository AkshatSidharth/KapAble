...
class CoolifyRequestError extends KapableError {
  readonly status: number;

  constructor(message: string, kind: KapableErrorKind, status: number) {
    super(message, kind);
 ...
 * itself, and a field KapAble does not send is one Coolify keeps as configured
...
export class CoolifyClient {
  ...
  private transportError(err: unknown): KapableError {
    ...
    if (!Array.isArray(body)) {
      throw new KapableError(
        `Coolify did not return a list of ${what}. This instance may be a ` +
          `version KapAble does not understand.`,
        KapableErrorKind.External,
      );
    }
    const parsed: T[] = [];
    ...
    if (parsed.length === 0 && body.length > 0) {
      throw new KapableError(
        `Coolify returned ${what} in an unexpected shape. This instance may ` +
          `be a version KapAble does not understand.`,
        KapableErrorKind.External,
      );
    }
    return parsed;
  ...
  async updateApplication(
    ...
    if (patch.build) {
      ...
      // Only what KapAble actually has a value for. A field it says nothing
