...
const EXPECTED_MANAGED_NODE_VERSION =
  IS_TEST_BUILD && process.env.KAPABLE_TEST_MANAGED_NODE_EXPECTED_VERSION
    ? process.env.KAPABLE_TEST_MANAGED_NODE_EXPECTED_VERSION
...
export class ManagedNodeInstallError extends KapableError {
...
function createManagedNodeInstallCancelledError(): KapableError {
...
function getManagedNodeArtifact(): ManagedNodeArtifact | null {
  const testArchiveUrl = IS_TEST_BUILD
    ...
    : undefined;
  if (testArchiveUrl) {
    let fileName = "kapable-test-managed-node.tar.gz";
    try {
    ...
    return {
      fileName,
      sha256: process.env.KAPABLE_TEST_MANAGED_NODE_SHA256 ?? "",
    };
  }
...
function getExpectedSha256(artifact: ManagedNodeArtifact): string {
  return IS_TEST_BUILD && process.env.KAPABLE_TEST_MANAGED_NODE_SHA256
    ? process.env.KAPABLE_TEST_MANAGED_NODE_SHA256
    : artifact.sha256;
}
...
async function installManagedNodeInternal(
  ...
  if (!artifact) {
    throw new ManagedNodeInstallError(
      "KapAble-managed Node.js is currently available on Windows and macOS.",
      "unsupported",
    );
  }
