...
vi.mock("@/paths/paths", () => ({
  getKapableAppPath: (appPath: string) => appPath,
}));
...
describe("cleanupOldMediaFiles filesystem safety", () => {
  ...
  it("does not touch external files through a symlinked media directory", async () => {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "kapable-media-"));
    tempDirectories.push(root);
    const appPath = path.join(root, "app");
    const externalPath = path.join(root, "external");
    await fs.mkdir(path.join(appPath, ".kapable"), { recursive: true });
    ...
    await fs.symlink(externalPath, path.join(appPath, KAPABLE_MEDIA_DIR_NAME));
  ...
  it("deletes stale media, keeps fresh media, and prunes the manifest", async () => {
    ...
    tempDirectories.push(root);
    const appPath = path.join(root, "app");
    const mediaPath = path.join(appPath, KAPABLE_MEDIA_DIR_NAME);
