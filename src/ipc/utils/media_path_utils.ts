...
/**
 * The root ".kapable" directory within each app that holds KapAble-managed files.
 */
export const KAPABLE_INTERNAL_DIR_NAME = ".kapable";

/**
 * The ".kapable"-relative subdir for uploaded media files.
 */
export const KAPABLE_MEDIA_SUBDIR = "media";

/**
 * The ".kapable"-relative subdir for screenshot files.
 */
export const KAPABLE_SCREENSHOT_SUBDIR = "screenshot";

/**
 * The subdirectory within each app where uploaded media files are stored.
 */
export const KAPABLE_MEDIA_DIR_NAME = `${KAPABLE_INTERNAL_DIR_NAME}/${KAPABLE_MEDIA_SUBDIR}`;
 ...
 */
export const KAPABLE_SCREENSHOT_DIR_NAME = `${KAPABLE_INTERNAL_DIR_NAME}/${KAPABLE_SCREENSHOT_SUBDIR}`;
...
/**
 * Check if an absolute path falls within the app's .kapable/media directory.
 * Used to validate that file copy operations only read from the allowed media dir.
 */
export function isWithinKapableMediaDir(
  absPath: string,
  appPath: string,
): boolean {
  const resolved = path.resolve(absPath);
  const resolvedMediaDir = path.resolve(
    path.join(appPath, KAPABLE_MEDIA_DIR_NAME),
  );
...
/**
 * Check if an absolute path is a file inside a .kapable/media directory
 * (without requiring a known app path). Validates by finding consecutive
 * ".kapable" + "media" path segments with at least one segment (filename) after,
 * then confirms the resolved path doesn't escape via ".." traversal.
 */
export function isFileWithinAnyKapableMediaDir(absPath: string): boolean {
  const resolved = path.resolve(absPath);
  const segments = resolved.split(path.sep);

  let mediaIdx = -1;
  for (let i = 0; i < segments.length - 2; i++) {
    if (segments[i] === ".kapable" && segments[i + 1] === "media") {
      mediaIdx = i + 1;
      break;
    }
  }
...
export function getKapableMediaDir(appPath: string): string {
  return path.join(appPath, KAPABLE_MEDIA_DIR_NAME);
}

export function getAttachmentsManifestPath(appPath: string): string {
  return path.join(getKapableMediaDir(appPath), ATTACHMENTS_MANIFEST_FILE);
}
...
async function pruneAttachmentManifestUnlocked(
  ...
  const mediaDir = getKapableMediaDir(appPath);
...
export async function resolveAttachmentLogicalPath(
  ...
  return toStoredAttachmentInfoIfPresent(getKapableMediaDir(appPath), entry);
}
