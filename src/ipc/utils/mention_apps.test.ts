...
vi.mock("@/paths/paths", () => ({
  getKapableAppPath: vi.fn((appPath: string) => `/kapable-apps/${appPath}`),
}));
...
describe("mention app utilities", () => {
  ...
  it("queries apps when the prompt has an app mention", async () => {
    ...
    expect(result.references).toEqual([
      { appName: "foo.app.com", appPath: "/kapable-apps/foo-app" },
    ]);
    ...
    expect(result.references).toEqual([
      {
        ...
        appPath: "/kapable-apps/spaced-app",
      },
    ]);
  ...
  it("resolves stored app paths fresh so renames and moves are picked up", async () => {
    ...
    expect(result.references).toEqual([
      { appName: "renamed", appPath: "/kapable-apps/moved-app" },
    ]);
    expect(result.changed).toBe(false);
  });
