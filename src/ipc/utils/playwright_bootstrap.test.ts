...
function makeAppWithBrowserMarker({
  ...
  const appPath = fs.mkdtempSync(path.join(os.tmpdir(), "kapable-pw-"));
...
describe("buildPreviewShimSource", () => {
  ...
  it("stays inert unless KapAble hands it an endpoint", () => {
    expect(source).toContain(`process.env.${PREVIEW_CDP_ENDPOINT_ENV}`);
    expect(source).toContain("!endpoint\n  ? pw.test");
  });

  it("attaches a screenshot of the page under test, not of KapAble", () => {
    // Playwright's own recorder shoots every page the connection can reach,
    // and over CDP that includes KapAble's windows — so a failing preview test
        ...
        await fixtures.page(
          ...
          async (usedPage) => {
            ...
            await api.post("/api/auth/sign-in/email", {
              data: { email: "test@kapable.test", password: "secret" },
            });
...
describe("ensurePreviewShim", () => {
  function makeApp(): string {
    const appPath = fs.mkdtempSync(path.join(os.tmpdir(), "kapable-pw-shim-"));
    tempDirs.push(appPath);
    return appPath;
  }
  ...
  it("writes the shim and the path mapping that reaches it", () => {
    ...
    expect(tsconfig.compilerOptions.paths["@playwright/test"]).toEqual([
      "./fixtures/kapable/kapable-test.ts",
    ]);
  ...
  it("removes the shim an older KapAble left at the fixtures root", () => {
    const appPath = makeApp();
    ...
    fs.writeFileSync(legacyShimPath, "// Generated by KapAble. old shim\n");
  ...
  it("keeps the old shim alive while the app's tsconfig still maps to it", () => {
    ...
    fs.mkdirSync(path.dirname(legacyShimPath), { recursive: true });
    fs.writeFileSync(legacyShimPath, "// Generated by KapAble. old shim\n");
    // The mapping an older KapAble's warning told the user to add. Deleting the
    // file underneath it would leave "@playwright/test" resolving to nothing —
    // breaking every run in the app, not just preview ones.
    fs.writeFileSync(
      tsconfigAt(appPath),
      '{ "compilerOptions": { "paths": { "@playwright/test": ["./fixtures/kapable-test.ts"] } } }',
    );
    ...
    expect(forwarder).toContain('export * from "./kapable/kapable-test"');
    expect(forwarder).not.toContain('from "@playwright/test"');
    expect(forwarder).not.toContain("connectOverCDP");
  });

  it("restores an old shim a previous KapAble deleted out from under the mapping", () => {
    const appPath = makeApp();
    ...
    fs.writeFileSync(
      tsconfigAt(appPath),
      '{ "compilerOptions": { "paths": { "@playwright/test": ["fixtures/kapable-test"] } } }',
    );
    ...
    expect(
      fs.readFileSync(
        path.join(appPath, "e2e-tests", "fixtures", "kapable-test.ts"),
        "utf8",
      ),
    ).toContain('export * from "./kapable/kapable-test"');
  ...
  it("refreshes its own files without asking", () => {
    const appPath = makeApp();
    ensurePreviewShim(appPath);
    fs.writeFileSync(shimAt(appPath), "// Generated by KapAble. stale contents\n");
  ...
  it("warns instead of hijacking a tsconfig the app owns", () => {
    ...
    expect(warning).toContain("./fixtures/kapable/kapable-test.ts");
    expect(
      fs.existsSync(path.join(appPath, "e2e-tests/fixtures/kapable/kapable-test.ts")),
    ).toBe(true);
  ...
  it("warns when the app's tsconfig only mentions the shim in passing", () => {
    ...
    fs.writeFileSync(
      tsconfigAt(appPath),
      '{ "include": ["./fixtures/kapable/kapable-test.ts"], "compilerOptions": {} }',
    );
  ...
  it("stays quiet when the app's own tsconfig already routes to the shim", () => {
    ...
    fs.writeFileSync(
      tsconfigAt(appPath),
      '{ "compilerOptions": { "paths": { "@playwright/test": ["./fixtures/kapable/kapable-test.ts"] } } }',
    );
...
describe("ensurePlaywrightBootstrap", () => {
  ...
  it("writes its own config and never touches the app's playwright.config.ts", async () => {
    ...
    const kapableConfigPath = path.join(appPath, KAPABLE_CONFIG_FILENAME);
    expect(fs.existsSync(kapableConfigPath)).toBe(true);
    expect(fs.readFileSync(kapableConfigPath, "utf8")).toContain(
      TEST_BASE_URL_ENV,
    );
    // The user's config survives byte-for-byte, with no backup left behind —
    // KapAble no longer takes over the canonical config name.
    expect(fs.readFileSync(userConfigPath, "utf8")).toBe(userConfig);
    expect(fs.existsSync(`${userConfigPath}.backup`)).toBe(false);
  });

  it("migrates an older KapAble-generated config's testDir to ./e2e-tests", async () => {
    const { appPath } = makeAppWithBrowserMarker({
      packageVersion: "1.2.3",
      executableExists: true,
    });
    const configPath = path.join(appPath, KAPABLE_CONFIG_FILENAME);
    fs.writeFileSync(
      configPath,
      'import { defineConfig } from "@playwright/test";\n' +
        "// Generated by KapAble.\n" +
  ...
  it("scopes an older KapAble-generated config's recorders away from preview runs", async () => {
    ...
    // by an older KapAble still captures every page reachable over the borrowed
    // CDP connection — which includes KapAble's own product windows.
    const { appPath } = makeAppWithBrowserMarker({
      packageVersion: "1.2.3",
      executableExists: true,
    });
  ...
  it("teaches an older KapAble-generated config the slow-motion option", async () => {
    const { appPath } = makeAppWithBrowserMarker({
      packageVersion: "1.2.3",
      executableExists: true,
    });
    ...
    // bootstrapped by an older KapAble.
    expect(updated).toContain(
      `launchOptions: { slowMo: Number(process.env.${TEST_SLOW_MO_ENV}) || 0 }`,
    );
  ...
  it("skips the browser download when the preview shim is routed", async () => {
    ...
    fs.writeFileSync(
      path.join(appPath, KAPABLE_CONFIG_FILENAME),
  ...
  it("leaves a config without the KapAble sentinel untouched", async () => {
    const { appPath } = makeAppWithBrowserMarker({
      packageVersion: "1.2.3",
      executableExists: true,
    });
    const configPath = path.join(appPath, KAPABLE_CONFIG_FILENAME);
    // No "Generated by KapAble" sentinel — the user has made this file their own.
    const userOwned =
      'import { defineConfig } from "@playwright/test";\n' +
      'export default defineConfig({ testDir: "./tests" });\n';
  ...
  it("is a no-op when a KapAble config already targets ./e2e-tests", async () => {
    const { appPath } = makeAppWithBrowserMarker({
      packageVersion: "1.2.3",
      executableExists: true,
    });
  ...
  it("points the package.json test script at KapAble's config", async () => {
    const { appPath } = makeAppWithBrowserMarker({
      packageVersion: "1.2.3",
      executableExists: true,
    });
    ...
    expect(pkg.scripts.test).toBe(
      `playwright test --config ${KAPABLE_CONFIG_FILENAME}`,
    );
  });

  it("migrates the old KapAble-generated bare test script", async () => {
    const { appPath } = makeAppWithBrowserMarker({
      packageVersion: "1.2.3",
      executableExists: true,
    });
  ...
  it("leaves a bare test script alone when the app owns a playwright.config", async () => {
    ...
    // and global setup, and break `npm test` outside KapAble.
    fs.writeFileSync(
...
describe("preview gitignore entries", () => {
  function makeApp(): string {
    const appPath = fs.mkdtempSync(path.join(os.tmpdir(), "kapable-pw-ignore-"));
    tempDirs.push(appPath);
    return appPath;
  }
  ...
  it("leaves an app-owned e2e tsconfig tracked", () => {
    ...
    // The shim directory is KapAble's either way.
    expect(gitignoreOf(appPath)).toContain("/e2e-tests/fixtures/kapable/");
  });
});

describe("app-owned tsconfig routing", () => {
  function makeApp(): string {
    const appPath = fs.mkdtempSync(path.join(os.tmpdir(), "kapable-pw-extends-"));
    tempDirs.push(appPath);
    return appPath;
  }
  ...
  it("accepts a mapping the app inherits through extends", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        compilerOptions: {
          paths: { "@playwright/test": ["./fixtures/kapable/kapable-test.ts"] },
        },
      }),
    );
    fs.writeFileSync(
...
describe("configSetsTimeout", () => {
  function makeAppWithConfig(source: string): string {
    const appPath = fs.mkdtempSync(path.join(os.tmpdir(), "kapable-pw-timeout-"));
    tempDirs.push(appPath);
    fs.writeFileSync(path.join(appPath, KAPABLE_CONFIG_FILENAME), source);
    return appPath;
  }
