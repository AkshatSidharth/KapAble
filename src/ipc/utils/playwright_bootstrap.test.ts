...
describe("ensurePreviewShim", () => {
  ...
  it("refreshes its own files without asking", () => {
    ...
    fs.writeFileSync(
      shimAt(appPath),
      "// Generated by KapAble. stale contents\n",
    );
  ...
  it("warns instead of hijacking a tsconfig the app owns", () => {
    ...
    expect(
      fs.existsSync(
        path.join(appPath, "e2e-tests/fixtures/kapable/kapable-test.ts"),
      ),
    ).toBe(true);
...
describe("preview gitignore entries", () => {
  function makeApp(): string {
    const appPath = fs.mkdtempSync(
      path.join(os.tmpdir(), "kapable-pw-ignore-"),
    );
    tempDirs.push(appPath);
    return appPath;
  }
...
describe("app-owned tsconfig routing", () => {
  function makeApp(): string {
    const appPath = fs.mkdtempSync(
      path.join(os.tmpdir(), "kapable-pw-extends-"),
    );
    tempDirs.push(appPath);
    return appPath;
  }
...
describe("configSetsTimeout", () => {
  function makeAppWithConfig(source: string): string {
    const appPath = fs.mkdtempSync(
      path.join(os.tmpdir(), "kapable-pw-timeout-"),
    );
    tempDirs.push(appPath);
