...
import { getKapableMediaDir } from "@/ipc/utils/media_path_utils";
import {
...
async function spillOutput(params: {
  ...
  const outputPath = path.join(
    getKapableMediaDir(params.appPath),
...
export async function executeSandboxScriptInProcess(
  ...
  let wallClockTimeoutError: KapableError | undefined;
  ...
  try {
    ...
    const context = new ExecutionContext({
      ...
      snapshotKey: `kapable-sandbox:${params.appPath}`,
    });
    ...
    const result = await Promise.race([
      ...
      new Promise<never>((_, reject) => {
        wallClockTimeout = setTimeout(() => {
          wallClockTimeoutError = new KapableError(
