...
describe("sandbox capabilities", () => {
  ...
  beforeEach(async () => {
    appPath = await fs.mkdtemp(path.join(os.tmpdir(), "kapable-sandbox-"));
  ...
  it("rejects sandbox writes that escape the app through symlinks", async () => {
    const outsideDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "kapable-outside-"),
    );
    try {
  ...
  it("allows reading and listing under .kapable/", async () => {
    await expect(
      sandboxReadFile(appPath, ".kapable/media/stored-log.txt"),
    ).resolves.toBe("line1\nline2\n");
    await expect(
      sandboxReadFile(appPath, ".kapable/media/attachments-manifest.json"),
    ).resolves.toContain("server.log");
    await expect(sandboxListFiles(appPath, ".kapable/media")).resolves.toEqual(
      expect.arrayContaining([
        ".kapable/media/attachments-manifest.json",
        ".kapable/media/stored-log.txt",
      ]),
    );
  });
