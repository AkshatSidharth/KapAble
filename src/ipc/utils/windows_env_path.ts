...
export function mergeWindowsPathSegments(
  ...
  // the shell that launched KapAble), then use the freshly read registry ordering
  // for every registry-known entry. This preserves Windows machine-before-user
  // precedence when a new machine PATH entry appears while KapAble is running.
 ...
 * installs Node.js while KapAble is running, the installer updates PATH in the
 ...
 * only way to pick up the new entries without restarting KapAble.
 */
