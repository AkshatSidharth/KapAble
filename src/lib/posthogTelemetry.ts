...
const POSTHOG_ERROR_DEDUPE_STORAGE_KEY = "kapablePostHogErrorDedupe:v1";
...
export function getSettingsPersonTelemetryProperties(settings: UserSettings) {
  return {
    isPro: hasKapableProKey(settings),
...
export class PostHogErrorDeduper {
  ...
  process<T extends PostHogTelemetryEvent | null | undefined>(
    ...
    return {
      ...
      properties: {
        ...event.properties,
        kapable_error_suppressed_count: suppressedCount,
        kapable_error_suppression_duration_ms: Math.max(
          0,
          now - existing.lastSentAt,
        ),
      },
