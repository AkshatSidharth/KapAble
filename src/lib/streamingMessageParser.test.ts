...
describe("streamingMessageParser", () => {
  ...
  it("parses a single kapable-write tag", () => {
    const content =
      'Before\n<kapable-write path="src/foo.ts" description="foo">code here</kapable-write>\nAfter';
    const { blocks } = parseFullMessage(content);
  ...
  it("parses an inline sub-agent mount point", () => {
    const content =
      '<kapable-subagent chat-id="7" thread-id="explorer-1" persona="explorer" task-name="Trace auth"></kapable-subagent>';
    const { blocks } = parseFullMessage(content);

    expect(blocksToShape(blocks)).toEqual([
      {
        kind: "custom-tag",
        tag: "kapable-subagent",
        attributes: {
          "chat-id": "7",
  ...
  it("handles xml-escaped attribute and content values", () => {
    const content =
      '<kapable-write path="a.ts" description="A &amp; B">if (a &lt; b) {}</kapable-write>';
    const { blocks } = parseFullMessage(content);
    expect(blocks).toHaveLength(1);
    const tag = blocks[0];
  ...
  it("treats unclosed opening tag as in-progress", () => {
    const content = '<kapable-write path="x.ts">partial';
    const { blocks } = parseFullMessage(content);
    expect(blocks).toHaveLength(1);
    expect(blocks[0]).toMatchObject({
      kind: "custom-tag",
  ...
  it("surfaces partial closing-tag bytes in the open block content", () => {
    // Stream stops mid-closing-tag. The buffered "</kapable-wri" bytes must
    // appear in the visible content so they stream and aren't lost.
    const cases = [
      { suffix: "<", expected: "content<" },
      { suffix: "</", expected: "content</" },
      { suffix: "</kapable-wri", expected: "content</kapable-wri" },
    ];
    for (const { suffix, expected } of cases) {
      const content = `<kapable-write path="x.ts">content${suffix}`;
      const { blocks } = parseFullMessage(content);
      expect(blocks).toHaveLength(1);
      expect(blocks[0]).toMatchObject({
        kind: "custom-tag",
...
<kapable-write path="src/foo.ts" description="foo &amp; bar">
const x = 1;
if (a &lt; b) { console.log("&amp;"); }
</kapable-write>
...
Final words.`;
  ...
  it("preserves committed block refs across updates", () => {
    const part1 =
      'Before\n<kapable-write path="x.ts">done</kapable-write>\n<kapable-write path="y.ts">in';
    ...
    // Completed kapable-write for x.ts should keep its identity.
  ...
  it("only the open block changes ref across many small chunks (O(chunk) renderer guarantee)", () => {
    // 50 completed kapable-write blocks followed by one open block.
    const completedSegments: string[] = [];
    for (let i = 0; i < 50; i++) {
      completedSegments.push(
        `<kapable-write path="f${i}.ts">content ${i}</kapable-write>`,
      );
    }
    const baseContent =
      completedSegments.join("\n") + '\n<kapable-write path="open.ts">';
  ...
  it("handles a closing-tag that doesn't match the open tag as content", () => {
    ...
    expect(blocks[0]).toMatchObject({
      kind: "custom-tag",
      tag: "kapable-write",
      content: "foo</kapable-edit>still",
      complete: true,
    });
  });
  ...
  it("random-split fuzz: incremental matches full parse for many seeds", () => {
    const content = `Hello.
<kapable-write path="a.ts" description="x">code &lt;tag&gt; here</kapable-write>
mid prose
<kapable-add-dependency packages="react vue"></kapable-add-dependency>
<think>analysis &amp; plan</think>
<kapable-write path="b.ts">b body
multi-line
content</kapable-write>
trailing`;
