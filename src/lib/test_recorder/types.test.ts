...
describe("parseRecorderAction", () => {
  ...
  it("rejects a navigation that leaves the app", () => {
    ...
    expect(
      parseRecorderAction({ kind: "navigate", path: "//kapable.invalid/x" }),
    ).toBeNull();
  });
  ...
  it("rejects a navigation hiding its authority behind a control character", () => {
    ...
    // these are all the authority-relative `//kapable.invalid/x` by the time
    ...
    for (const separator of ["\t", "\n", "\r"]) {
      expect(
        parseRecorderAction({
          kind: "navigate",
          path: `/${separator}/kapable.invalid/x`,
        }),
      ).toBeNull();
      expect(
        parseRecorderAction({
