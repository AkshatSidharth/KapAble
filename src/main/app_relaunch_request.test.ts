...
describe("app relaunch request", () => {
  ...
  it("coalesces generic reopen requests into one relaunch", () => {
    ...
    request.finish({
      currentArgs: ["--original-argument", "kapable://stale"],
      relaunch,
      quit,
    });
  ...
  it("preserves a shutdown-time deep link in the replacement process", () => {
    const request = createAppRelaunchRequest();
    const relaunch = vi.fn();
    const deepLinkUrl = "kapable://oauth-return?code=new";
    ...
    request.finish({
      currentArgs: [
        "/path/to/main.js",
        "--original-argument",
        "kapable://oauth-return?code=stale",
      ],
      relaunch,
      quit: vi.fn(),
    });
  ...
  it("keeps the first deep link when shutdown receives repeated requests", () => {
    ...
    request.request({ deepLinkUrl: "kapable://first" });
    request.request({ deepLinkUrl: "kapable://second" });
    request.finish({ currentArgs: [], relaunch, quit: vi.fn() });

    expect(relaunch).toHaveBeenCalledWith({ args: ["kapable://first"] });
  });
});
