...
import {
  KAPABLE_INTERNAL_DIR_NAME,
  KAPABLE_MEDIA_SUBDIR,
  KAPABLE_SCREENSHOT_SUBDIR,
...
type KapableMediaProtocolDependencies = {
  cacheRoot: string;
...
async function resolveContainedMediaPath(
  ...
  const targetDir = path.resolve(
    path.join(appPath, KAPABLE_INTERNAL_DIR_NAME, subdir),
  );
  ...
  // Do not let .kapable or its media subdirectory redirect the protocol to a
  ...
  // Prevent a symlink inside .kapable/media from escaping the app directory.
...
export function createKapableMediaProtocolHandler({
  cacheRoot,
  resolveAppPath,
  resolveAppId,
  fetchFile,
  createThumbnailFromPath,
}: KapableMediaProtocolDependencies) {
  ...
  return async (request: Request): Promise<Response> => {
    try {
      ...
      if (url.protocol !== "kapable-media:" || url.hostname !== "media") {
        return response(403, "Forbidden");
      }

      // Formats:
      // kapable-media://media/{app-path}/.kapable/{subdir}/{filename}
      // kapable-media://media/app-id/{id}/.kapable/{subdir}/{filename}
      const pathSegments = url.pathname.slice(1).split("/");
      const allowedSubdirs = [KAPABLE_MEDIA_SUBDIR, KAPABLE_SCREENSHOT_SUBDIR];
      const usesAppId =
      ...
      if (
        ...
        pathSegments[internalDirIndex] !== KAPABLE_INTERNAL_DIR_NAME ||
      ...
      if (subdir !== KAPABLE_MEDIA_SUBDIR) {
        return response(403, "Forbidden");
      }
        ...
        headers: {
          ...
          "Cache-Control":
            requestedVersion === thumbnail.sourceVersion
              ...
              : "no-store",
          "X-KapAble-Thumbnail-Cache": thumbnail.cacheHit ? "hit" : "miss",
        },
      });
    } catch (error) {
