...
/**
 * Per-chat queued-prompt files live inside the app's KapAble-managed `.kapable/`
 * folder, mirroring how agent todos are persisted
 * (`<appPath>/.kapable/todos/<chatId>.json`). Keeping them here means they are
 * scoped to their app and cleaned up automatically when the app is deleted.
 *
 * Layout: `<appPath>/.kapable/queue/<chatId>.json`
 */
function getChatQueueDir(appPath: string): string {
  return path.join(appPath, ".kapable", "queue");
}
...
function listAppPaths(): string[] {
  return getDb()
    ...
    .from(apps)
    .all()
    .map((row) => getKapableAppPath(row.path));
}
