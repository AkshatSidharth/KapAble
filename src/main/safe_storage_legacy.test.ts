...
function assertInProcessKeychainReaderAddonAvailable(): void {
  try {
    require("kapable-keychain-reader");
  } catch (error) {
    throw new Error(
      "Failed to load kapable-keychain-reader native addon. " +
        "Run `npm rebuild kapable-keychain-reader` before running macOS " +
...
function loadNativeKeychainReaderBinding(): NativeKeychainReaderBinding {
  return require("kapable-keychain-reader") as NativeKeychainReaderBinding;
}

const KAPABLE_KEY = "kapable Safe Storage::kapable Key";
...
describe("InProcessKeychainPasswordReader", () => {
  ...
  it("caches hits and null misses per identity", async () => {
    ...
    setInProcessKeychainBindingLoaderForTesting(() => ({
      readGenericPassword(service, account, keychainPath, allowUI) {
        ...
        if (service === "kapable Safe Storage" && account === "kapable Key") {
    ...
    await withPlatform("darwin", () => {
      ...
      expect(reader.readPassword("kapable Safe Storage", "kapable Key")).toBe(
        "stored-password",
      );
      expect(reader.readPassword("kapable Safe Storage", "kapable Key")).toBe(
        "stored-password",
  ...
  it("env var selects CLI vs in-process default reader", () => {
    const savedReader = process.env.KAPABLE_SAFE_STORAGE_READER;
    try {
      delete process.env.KAPABLE_SAFE_STORAGE_READER;
      expect(createDefaultKeychainPasswordReaderForTesting()).toBeInstanceOf(
        InProcessKeychainPasswordReader,
      );

      process.env.KAPABLE_SAFE_STORAGE_READER = "cli";
      expect(createDefaultKeychainPasswordReaderForTesting()).toBeInstanceOf(
        SecurityCliKeychainPasswordReader,
      );
    } finally {
      if (savedReader === undefined) {
        ...
        process.env.KAPABLE_SAFE_STORAGE_READER = savedReader;
      }
    }
  });
...
describe("Keychain unlock recovery retry", () => {
  ...
  const savedPromptKillSwitch =
    process.env.KAPABLE_DISABLE_SAFE_STORAGE_UNLOCK_PROMPT;
  ...
  it("honors the unlock-prompt kill switch", async () => {
    ...
    await withPlatform("darwin", () => {
      expect(recoverLegacySafeStorageSecret(ciphertext)).toBeNull();
      process.env.KAPABLE_DISABLE_SAFE_STORAGE_UNLOCK_PROMPT = "1";
      expect(recoveryNeedsKeychainUnlock()).toBe(false);
...
describe("recoverLegacySafeStorageSecret", () => {
  const savedKillSwitch = process.env.KAPABLE_DISABLE_SAFE_STORAGE_RECOVERY;
  const savedReader = process.env.KAPABLE_SAFE_STORAGE_READER;
  ...
  afterEach(() => {
    if (savedKillSwitch === undefined) {
      ...
      process.env.KAPABLE_DISABLE_SAFE_STORAGE_RECOVERY = savedKillSwitch;
    }
    if (savedReader === undefined) {
      delete process.env.KAPABLE_SAFE_STORAGE_READER;
    } else {
  ...
  it("recovers via the kapable identity", async () => {
    const key = deriveLegacyOsCryptKey("kapable-keychain-pw");
    const ciphertext = encryptV10Base64("kapable-real-secret", key);
    const reader = makeFakeReader({ [KAPABLE_KEY]: "kapable-keychain-pw" });
  ...
  it("recovers via the chromium identity when the kapable identity misses", async () => {
  ...
  it("rejects a wrong-key decrypt that unpads to invalid UTF-8 and tries the next identity", async () => {
    ...
    const reader = makeFakeReader({
      [KAPABLE_KEY]: falsePositivePw!,
      [CHROMIUM_KEY]: "correct-chromium-pw",
    });

    await withPlatform("darwin", () => {
      // The kapable identity "decrypts" to garbage and is rejected; the chromium
  ...
  it("preserves ciphertext when multiple identities decrypt plausibly", async () => {
    ...
    const reader = makeFakeReader({
      [KAPABLE_KEY]: "shared-keychain-pw",
  ...
  it("returns null under the kill switch and never reads the keychain", async () => {
    process.env.KAPABLE_DISABLE_SAFE_STORAGE_RECOVERY = "1";
    const key = deriveLegacyOsCryptKey("pw");
    const ciphertext = encryptV10Base64("secret", key);
    const reader = makeFakeReader({ [KAPABLE_KEY]: "pw" });
...
describe.skipIf(process.platform !== "darwin")(
  ...
  () => {
    const tmpDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "kapable-safe-storage-legacy-"),
    );
    const keychainPath = path.join(tmpDir, "kapable-recovery-test.keychain");
...
describe.skipIf(process.platform !== "darwin")(
  ...
  () => {
    const tmpDir = fs.mkdtempSync(
      path.join(os.tmpdir(), "kapable-safe-storage-in-process-"),
    );
    const keychainPath = path.join(tmpDir, "kapable-recovery-test.keychain");
    const keychainPassword = "testpass";
    const storedPassword = "integration-test-password";
    const promptService = "kapable Prompt Test Safe Storage";
    ...
    it("returns null promptly instead of prompting when Keychain UI would be required", () => {
      ...
      expect(reader.readPassword(promptService, "kapable Key")).toBeNull();
