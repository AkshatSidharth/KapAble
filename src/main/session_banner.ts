...
// session ends and the next begins. Grep for "===== KapAble " to find them all.
//
...
export function formatStartBanner({
  ...
  return `${MARKER} KapAble started | pid ${pid} | ${version} | ${platform} ${arch} | electron ${electron} | node ${node} ${MARKER}`;
}
...
export function formatExitBanner(
  ...
  return `${MARKER} KapAble exiting | pid ${pid} | ${reason}${uptime} ${MARKER}`;
}
...
export function formatErrorBanner(
  ...
  return `${MARKER} KapAble main-process error | pid ${pid} | ${kind} | uptime ${formatUptime(uptimeMs)} ${MARKER}`;
}
...
export function formatPreviousSessionBanner(
  ...
  return `${MARKER} KapAble previous session ended unexpectedly | ${started}${formatPreviousUptime(startedAt, lastSeenAt)} ${MARKER}`;
}
