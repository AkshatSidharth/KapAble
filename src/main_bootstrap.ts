...
function reportRuntimeLoadFailure(error: unknown): void {
  logger.error("Failed to load the KapAble application runtime:", error);
  const logPath = log.transports.file.getFile().path;
  dialog.showErrorBox(
    "KapAble failed to start",
...
// free of application imports so those hooks cannot start normal KapAble services.
if (started) {
  app.quit();
} else {
