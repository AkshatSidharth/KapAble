...
export function transition(
  ...
  switch (event.type) {
      ...
      return applied(
        {
          ...
          // Restoring it as "kapable" regardless would hand a redirect destination
    ...
    case "NAVIGATED_IN_APP": {
      ...
      // A document load caused by KapAble's own navigation must not be misread as
    ...
    case "SYNC_ERROR":
      if (state.error && state.error.source !== "kapable-sync") {
        return ignore(state, "higher-priority-error");
      }
      if (
        state.error?.source === "kapable-sync" &&
        state.error.message === event.message
      ) {
        return ignore(state, "same-preview-error");
      }
      return applied({
        ...state,
        error: { message: event.message, source: "kapable-sync" },
      });
    case "SYNC_RECOVERED":
      return state.error?.source === "kapable-sync"
        ? applied({ ...state, error: undefined })
        : ignore(state, "no-sync-error");
    case "APP_ERROR":
      if (
        state.error?.source === "kapable-app" &&
        state.error.message === event.message
      ) {
        return ignore(state, "same-preview-error");
      }
      return applied({
        ...state,
        error: { message: event.message, source: "kapable-app" },
      });
    ...
    case "PICKER_TOGGLED": {
      ...
      commands.push({
        ...
        message: {
          type: picking
            ? "activate-kapable-component-selector"
            : "deactivate-kapable-component-selector",
        },
      });
