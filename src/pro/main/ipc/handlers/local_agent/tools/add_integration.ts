...
> = {
  ...
  buildXml: (args, _isComplete) => {
    ...
    if (args.provider && args.provider !== "none") {
      return `<kapable-add-integration provider="${escapeXmlAttr(args.provider)}" outcome="pending"></kapable-add-integration>`;
    }
    return `<kapable-add-integration outcome="pending"></kapable-add-integration>`;
  },
  ...
  execute: async (args, ctx: AgentContext) => {
    ...
    if (
      ...
      ctx.onXmlComplete(
        `<kapable-add-integration outcome="dismissed"></kapable-add-integration>`,
      );
    ...
    if (!result.completed && result.provider === null) {
      ctx.onXmlComplete(
        `<kapable-add-integration outcome="skipped"></kapable-add-integration>`,
      );
    ...
    ctx.onXmlComplete(
      `<kapable-add-integration provider="${escapeXmlAttr(result.provider)}" outcome="completed"></kapable-add-integration>`,
    );
