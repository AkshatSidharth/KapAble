...
const mocks = vi.hoisted(() => ({
  readSettings: vi.fn(),
  getKapableEngineBaseUrl: vi.fn(),
}));

vi.mock("@/main/settings", () => ({ readSettings: mocks.readSettings }));
vi.mock("@/ipc/utils/kapable_engine_url", () => ({
  getKapableEngineBaseUrl: mocks.getKapableEngineBaseUrl,
}));
...
describe("engineFetch", () => {
  beforeEach(() => {
    ...
    mocks.getKapableEngineBaseUrl.mockReturnValue("https://engine.test/v1");
  });
  ...
  it("does no useful work for an already-aborted caller signal", async () => {
    ...
    await expect(
      engineFetch({ kapableRequestId: "request-3" }, "/tools/test", {
        signal: controller.signal,
      }),
  ...
  it("keeps cancellation active until the response body settles", async () => {
    ...
    const response = await engineFetch(
      { kapableRequestId: "request-4", abortSignal: controller.signal },
      "/tools/test",
    );
  ...
  it("combines an explicit signal with the turn abort signal", async () => {
    ...
    const request = engineFetch(
      {
        kapableRequestId: "request-combined-signals",
  ...
  it("supports a per-endpoint timeout override", async () => {
    ...
    const request = engineFetch(
      { kapableRequestId: "request-short-timeout" },
      "/tools/test",
      { timeoutMs: 25 },
    );
    const rejection = expect(request).rejects.toMatchObject({
      message: "KapAble engine request to /tools/test timed out after 25ms",
      kind: KapableErrorKind.External,
    });
  ...
  it("uses the context abort signal by default after headers arrive", async () => {
    ...
    const response = await engineFetch(
      { kapableRequestId: "request-headers", abortSignal: controller.signal },
      "/tools/test",
    );
  ...
  it("applies the timeout until the response body settles", async () => {
    ...
    const response = await engineFetch(
      { kapableRequestId: "request-body-timeout" },
      "/tools/test",
    );
  ...
  it("cleans up when a caller abandons and cancels the response body", async () => {
    ...
    const response = await engineFetch(
      { kapableRequestId: "request-abandoned", abortSignal: controller.signal },
      "/tools/test",
    );
