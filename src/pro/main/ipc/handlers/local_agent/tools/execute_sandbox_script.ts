...
function isAttachmentHostCallPath(path: string | undefined): boolean {
  ...
  return (
    normalized === KAPABLE_MEDIA_DIR_NAME ||
    normalized.startsWith(`${KAPABLE_MEDIA_DIR_NAME}/`)
  );
}
...
function buildScriptXml(params: {
  ...
  return `<kapable-script ${attrs.join(" ")}>${escapeXmlContent(payload)}</kapable-script>`;
}
    ...
    execute: async (args: ExecuteSandboxScriptArgs, ctx: AgentContext) => {
      ...
      try {
        ...
        throw new KapableError(
          ...
          isKapableError(error) ? error.kind : KapableErrorKind.Validation,
        );
      }
    },
  };
