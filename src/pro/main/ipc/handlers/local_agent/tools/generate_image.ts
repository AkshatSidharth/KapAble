...
const DESCRIPTION = `Generate an image using AI based on a text prompt. The generated image is saved to the project's .kapable/media directory.
...
### After Generation
The tool returns the file path in .kapable/media. Use the copy_file tool to copy it to the appropriate location in the project (e.g., public/assets/) and reference that path in your code.
`;
...
async function saveGeneratedImage(
  ...
  const relativePath = path.join(KAPABLE_MEDIA_DIR_NAME, fileName);
...
> = {
  ...
  buildXml: (args, isComplete) => {
    ...
    return `<kapable-image-generation prompt="${escapeXmlAttr(args.prompt)}">`;
  },
  ...
  execute: async (args, ctx: AgentContext) => {
    ...
    ctx.onXmlStream(
      `<kapable-image-generation prompt="${escapeXmlAttr(args.prompt)}">`,
    );
    ...
    try {
      ...
      ctx.onXmlComplete(
        `<kapable-image-generation prompt="${escapeXmlAttr(args.prompt)}" path="${escapeXmlAttr(relativePath)}">${escapeXmlContent(relativePath)}</kapable-image-generation>`,
      );
      ...
      ctx.onXmlComplete(
        `<kapable-image-generation prompt="${escapeXmlAttr(args.prompt)}"></kapable-image-generation>`,
      );
      throw error;
    }
  },
