...
> = {
  ...
  execute: async (args, ctx: AgentContext) => {
    ...
    if (provider === "supabase" && canUseSupabaseTools(ctx)) {
      ctx.onXmlStream(
        `<kapable-db-table-schema provider="Supabase"${tableAttr}></kapable-db-table-schema>`,
      );
      ...
      ctx.onXmlComplete(
        `<kapable-db-table-schema provider="Supabase"${tableAttr}>\n${escapeXmlContent(schema)}\n</kapable-db-table-schema>`,
      );
    ...
    if (provider === "neon" && canUseNeonTools(ctx)) {
      ctx.onXmlStream(
        `<kapable-db-table-schema provider="Neon"${tableAttr}></kapable-db-table-schema>`,
      );
      ...
      ctx.onXmlComplete(
        `<kapable-db-table-schema provider="Neon"${tableAttr}>\n${escapeXmlContent(schema)}\n</kapable-db-table-schema>`,
      );
