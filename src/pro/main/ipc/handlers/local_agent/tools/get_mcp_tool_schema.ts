...
export const getMcpToolSchemaTool: ToolDefinition<GetMcpToolSchemaArgs> = {
  ...
  buildXml: (args, isComplete) => {
    ...
    return `<kapable-mcp-tool-schema tools="${escapeXmlAttr(tools.join(", "))}">Loading...`;
  },

  execute: async (args: GetMcpToolSchemaArgs, ctx: AgentContext) => {
    const finish = (result: string) => {
      ctx.onXmlComplete(
        `<kapable-mcp-tool-schema tools="${escapeXmlAttr(args.tools.join(", "))}">${escapeXmlContent(result)}</kapable-mcp-tool-schema>`,
      );
      return result;
    };
