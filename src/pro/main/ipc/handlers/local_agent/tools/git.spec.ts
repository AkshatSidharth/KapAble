...
describe("local-agent Git tool definitions", () => {
  ...
  it("captures durable presentation updates by tool call identity", async () => {
    ...
    expect(onToolActivity).toHaveBeenNthCalledWith(1, {
      ...
      toolName: "git_status",
      status: "pending",
      presentationXml: '<kapable-git operation="status">',
      inputJson: {},
    });
  ...
  it("records a successful mutation and skips reserved Supabase deployment", async () => {
    ...
    try {
      ...
      expect(ctx.onXmlComplete).toHaveBeenCalledWith(
        '<kapable-git operation="restore_file" revision="HEAD" path="supabase/functions/_shared/file.ts" not_staged="true"></kapable-git>',
      );
    } finally {
  ...
  it("renders an escaped pending preview but waits for execution to complete it", () => {
    expect(
      ...
      '<kapable-git operation="show_file" revision="HEAD" path="src/a&amp;b&quot;.ts">',
    );
    expect(
      gitShowFileTool.buildXml!(
  ...
  it("treats a dot path as the whole app for diff, log, and show commit", () => {
    expect(gitDiffTool.buildXml!({ path: "." }, false)).toBe(
      '<kapable-git operation="diff" scope="all">',
    );
    expect(gitLogTool.buildXml!({ path: "." }, false)).toBe(
      '<kapable-git operation="log" revision="HEAD" max_count="20">',
    );
    expect(
      gitShowCommitTool.buildXml!({ revision: "HEAD", path: "." }, false),
    ).toBe('<kapable-git operation="show_commit" revision="HEAD">');
