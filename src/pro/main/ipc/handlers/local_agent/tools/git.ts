...
function gitXml(
  ...
  const openingTag = `<kapable-git ${attributes.join(" ")}>`;
  if (options.complete === false) {
    return openingTag;
  }
  return `${openingTag}${options.content ? escapeXmlContent(options.content) : ""}</kapable-git>`;
}
...
export const gitDiffTool: ToolDefinition<z.infer<typeof gitDiffSchema>> = {
  name: "git_diff",
  description:
    "Show a bounded unified diff for tracked files in the current app. Untracked files are reported by git_status. Sensitive dotenv and KapAble-managed patches are omitted.",
...
> = {
  ...
  description:
    "Show metadata and a bounded first-parent patch for one commit in the current app. Optionally limit the patch to one path. Sensitive dotenv and KapAble-managed patches are omitted.",
