  ...
  for (const def of params.defs) {
    map[def.jsName] = async (rawArgs: unknown) => {
      ...
      const autoApprove = buildMcpAutoApprove({
        ...
        isKapablePro: params.ctx.isKapablePro,
      ...
      params.ctx.onXmlComplete(
        `<kapable-mcp-tool-call server="${escapeXmlAttr(def.serverName)}" tool="${escapeXmlAttr(def.toolName)}" call-id="${escapeXmlAttr(callId)}"${autoApprovedAttr}>\n${escapeXmlContent(contentPretty)}\n</kapable-mcp-tool-call>`,
      );
      ...
      try {
        ...
        params.ctx.onXmlComplete(
          `<kapable-mcp-tool-result server="${escapeXmlAttr(def.serverName)}" tool="${escapeXmlAttr(def.toolName)}" call-id="${escapeXmlAttr(callId)}">\n${escapeXmlContent(safeResult.serialized)}\n</kapable-mcp-tool-result>`,
        );
        ...
        // stuck on "Running".
        params.ctx.onXmlComplete(
          `<kapable-mcp-tool-result server="${escapeXmlAttr(def.serverName)}" tool="${escapeXmlAttr(def.toolName)}" call-id="${escapeXmlAttr(callId)}" is-error="true">\n${escapeXmlContent(safeErrorMessage)}\n</kapable-mcp-tool-result>`,
        );
        params.ctx.onXmlComplete(
          `<kapable-output type="error" message="MCP tool '${escapeXmlAttr(def.toolKey)}' failed: ${escapeXmlAttr(safeErrorMessage)}">${escapeXmlContent(safeErrorDetails)}</kapable-output>`,
        );
        throw error;
      }
    };
...
export function getMcpInlineTokenThreshold(): number {
  const raw = process.env.KAPABLE_MCP_INLINE_TOKEN_THRESHOLD;
  if (raw !== undefined) {
    const n = Number(raw);
