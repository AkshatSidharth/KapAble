...
describe("readChatTool.execute", () => {
  ...
  it("cleans payload tags and truncates long messages", async () => {
    ...
    harness.insertMessage({
      chatId,
      role: "assistant",
      content: `Intro prose. <kapable-write path="src/big.ts">${"SECRET".repeat(
        100,
      )}</kapable-write>`,
    });
    harness.insertMessage({
  ...
  it("emits a completed XML card with the range", async () => {
    ...
    expect(xml).toContain("<kapable-read-chat");
    expect(xml).toContain(`chat-id="${chatId}"`);
    expect(xml).toContain("range=");
  });

  it("throws a validation error via KapableError for unknown errors kinds", async () => {
    // Guard that notFound() is a KapableError instance (renderer contract).
