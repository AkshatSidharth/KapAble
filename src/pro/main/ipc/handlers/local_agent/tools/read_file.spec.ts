...
line 5`;
  ...
  beforeEach(async () => {
    ...
    const mediaDir = getKapableMediaDir(testDir);
  ...
  describe("buildXml", () => {
    ...
    it("builds XML with path only", () => {
      ...
      expect(result).toBe('<kapable-read path="src/App.tsx"></kapable-read>');
    });
    ...
    it("includes start_line attribute when provided", () => {
      ...
      expect(result).toBe(
        '<kapable-read path="src/App.tsx" start_line="10"></kapable-read>',
      );
    });
    ...
    it("includes end_line attribute when provided", () => {
      ...
      expect(result).toBe(
        '<kapable-read path="src/App.tsx" end_line="50"></kapable-read>',
      );
    });
    ...
    it("includes both line range attributes", () => {
      ...
      expect(result).toBe(
        '<kapable-read path="src/App.tsx" start_line="10" end_line="50"></kapable-read>',
      );
    });
    ...
    it("includes app_name attribute when provided", () => {
      ...
      expect(result).toBe(
        '<kapable-read path="src/App.tsx" app_name="other-app"></kapable-read>',
      );
    });
  });
  ...
  describe("execute - app_name (referenced apps)", () => {
    ...
    it("blocks .kapable/ paths when targeting a referenced app", async () => {
      mockContext.referencedApps.set("other-app", otherAppDir);
      await expect(
        readFileTool.execute(
          { path: ".kapable/chats/secret.md", app_name: "other-app" },
          mockContext,
        ),
      ).rejects.toThrow(/Cannot read \.kapable\/ paths from referenced apps/);
    });

    it("blocks .kapable/ paths reached via traversal aliases (e.g. src/../.kapable/...)", async () => {
      mockContext.referencedApps.set("other-app", otherAppDir);
      const kapableDir = path.join(otherAppDir, ".kapable");
      await fs.promises.mkdir(kapableDir, { recursive: true });
      await fs.promises.writeFile(
        path.join(kapableDir, "secret.md"),
      ...
      await expect(
        readFileTool.execute(
          { path: "src/../.kapable/secret.md", app_name: "other-app" },
          mockContext,
        ),
      ).rejects.toThrow(/Cannot read \.kapable\/ paths from referenced apps/);
    });
    ...
    it.runIf(process.platform !== "win32")(
      "blocks symlinks into a referenced app's protected .kapable directory",
      async () => {
        mockContext.referencedApps.set("other-app", otherAppDir);
        const kapableDir = path.join(otherAppDir, ".kapable");
        await fs.promises.mkdir(kapableDir, { recursive: true });
        const privateFile = path.join(kapableDir, "private.txt");
    ...
    it("allows .kapable/ paths on the current app (no app_name)", async () => {
      const kapableDir = path.join(testDir, ".kapable");
      await fs.promises.mkdir(kapableDir, { recursive: true });
      await fs.promises.writeFile(
        path.join(kapableDir, "notes.md"),
        "local kapable metadata",
      );
      const result = await readFileTool.execute(
        { path: ".kapable/notes.md" },
        mockContext,
      );
      expect(result).toBe("local kapable metadata");
    });
  });
