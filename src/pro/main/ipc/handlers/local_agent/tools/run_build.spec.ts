...
describe("run_build", () => {
  ...
  it("creates a Git-aware worktree with the live state of a nested app", async () => {
    ...
    try {
      ...
      await expect(
        fs.lstat(path.join(snapshot.worktreePath, ".kapable-build-snapshot")),
      ).rejects.toMatchObject({ code: "ENOENT" });
    } finally {
  ...
  it("cleans only marked KapAble-owned snapshot directories", async () => {
    ...
    const owned = path.join(snapshotRoot, ".kapable-build-ABC123");
    const unowned = path.join(snapshotRoot, ".kapable-build-DEF456");
    ...
    await fs.writeFile(
      `${owned}.owner.json`,
      JSON.stringify({
        schema: "kapable-build-worktree-v1",
