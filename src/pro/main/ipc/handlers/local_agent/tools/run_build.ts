...
const SNAPSHOT_PREFIX = ".kapable-build-";
const SNAPSHOT_NAME_PATTERN = /^\.kapable-build-[A-Za-z0-9]{6}$/;
const SNAPSHOT_MARKER_SUFFIX = ".owner.json";
const SNAPSHOT_MARKER_SCHEMA = "kapable-build-worktree-v1";
...
function throwIfBuildCancelled(signal?: AbortSignal): void {
  if (signal?.aborted) {
    throw new KapableError("Build cancelled.", KapableErrorKind.UserCancelled);
  }
}
...
function externalSnapshotLinkError(relativePath: string): KapableError {
...
function streamBuildOutput(ctx: AgentContext, accumulatedOutput: string): void {
  ...
  if (!output) return;
  ctx.onXmlStream(
    `<kapable-status title="Production build output">\n${escapeXmlContent(output)}\n</kapable-status>`,
  );
}
...
- The active preview stays running. Builds that could interfere with it are verified in an isolated workspace with clean dependencies.`,
  ...
  buildXml: (_args, isComplete) =>
    isComplete
      ? undefined
      : '<kapable-status title="Running production build"></kapable-status>',
    ...
    try {
      return await appOperationCoordinator.run(
        ...
        async () => {
          ...
          ctx.onXmlStream(
            `<kapable-status title="${escapeXmlAttr(mode === "in-place" ? "Building beside preview" : "Preparing isolated build")}"></kapable-status>`,
          );
          ...
          try {
            ...
            if (mode === "isolated") {
              ...
              ctx.onXmlStream(
                '<kapable-status title="Installing isolated build dependencies"></kapable-status>',
              );
