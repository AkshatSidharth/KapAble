...
describe("runTypeChecksTool precondition guidance", () => {
  ...
  async function makeApp(packageJson: object): Promise<string> {
    const appPath = await fs.mkdtemp(path.join(os.tmpdir(), "kapable-tsc-"));
  ...
  function expectWarningOutput(ctx: AgentContext): string {
    ...
    expect(output).toContain(
      '<kapable-output type="warning" message="Type checking unavailable">',
    );
    expect(output).not.toContain("<kapable-status");
  ...
  it("tells the agent to rebuild when TypeScript is declared but missing", async () => {
    ...
    expect(result).not.toContain('<kapable-command type="rebuild">');
  ...
  it("falls back to a user command when reinstall_and_restart_app is unavailable", async () => {
    ...
    expect(result).toContain('<kapable-command type="rebuild">');
  ...
  it("tells the agent not to retry and to suggest adding TypeScript for plain JavaScript projects", async () => {
    ...
    expect(result).toContain('<kapable-command type="add-typescript">');
    expect(expectWarningOutput(ctx)).toContain(
      '&lt;kapable-command type="add-typescript"',
    );
  });
  ...
  it("warns that configuration errors prevented a complete scoped check", async () => {
    ...
    expect(ctx.onXmlComplete).toHaveBeenCalledWith(
      expect.stringContaining(
        '<kapable-status title="Type check incomplete" state="warning">',
      ),
    );
  });
  ...
  it("discloses project errors outside a requested path", async () => {
    ...
    expect(ctx.onXmlComplete).toHaveBeenCalledWith(
      expect.stringContaining(
        '<kapable-status title="Type errors found" state="finished">',
      ),
    );
  });
  ...
  it("always reports every project error when project-wide reporting is enabled", async () => {
    ...
    expect(ctx.onXmlStream).toHaveBeenCalledWith(
      '<kapable-status title="Type checking all files"></kapable-status>',
    );
  });
  ...
  it("describes a clean scoped check without claiming a project-wide result", async () => {
    ...
    expect(ctx.onXmlComplete).toHaveBeenCalledWith(
      expect.stringContaining(
        '<kapable-status title="Type check passed" state="finished">',
      ),
    );
  });
});
