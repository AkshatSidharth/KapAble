...
describe("searchChatsTool.execute", () => {
  ...
  it("skips a compaction-summary match that duplicates an original hit", async () => {
    ...
    harness.insertMessage({
      ...
      role: "assistant",
      content:
        "<kapable-compaction>We picked the pelican logging approach.</kapable-compaction>",
      createdAt: 4_000,
  ...
  it("does not match text that only appeared inside dropped payload bodies", async () => {
    ...
    harness.insertMessage({
      ...
      role: "assistant",
      content:
        '<kapable-write path="src/x.ts">const flamingo = "only in code";</kapable-write>',
    });
    ...
    harness.insertMessage({
      ...
      role: "assistant",
      content:
        '<kapable-search-chats query="x">ostrich retrieved excerpt</kapable-search-chats>',
    });
    await drainChatSearchIndexOnce();
  ...
  it("emits a completed XML card", async () => {
    ...
    expect(xml).toContain("<kapable-search-chats");
