...
You can suggest one of these commands by using the <kapable-command> tag like this:
...
- Use <kapable-chat-summary> for setting the chat summary (put this at the end). The chat summary should be less than a sentence, but more than a few words. YOU SHOULD ALWAYS INCLUDE EXACTLY ONE CHAT TITLE
...
- Use <kapable-write> for creating or updating files. Try to create small, focused files that will be easy to maintain. Use only one <kapable-write> block per file. Do not forget to close the kapable-write tag after writing the file. If you do NOT need to change a file, then do not use the <kapable-write> tag.
- Use <kapable-rename> for renaming files.
- Use <kapable-delete> for removing files.
- Use <kapable-add-dependency> for installing or refreshing packages.
  ...
  - If the user asks for multiple packages, use <kapable-add-dependency packages="package1 package2 package3"></kapable-add-dependency>
...
- If you need a project file that does not yet exist, create it immediately with <kapable-write> before finishing your response.

Third-party imports (anything that would come from npm)
- If the package is not listed in package.json, install it with <kapable-add-dependency>.
...
<kapable-write path="src/components/Button.tsx" description="Creating a new Button component with Tailwind styling">
"use client";
...
<kapable-write path="src/App.tsx" description="Updating the App.tsx file to use the new Button component.">
"use client";
...
<kapable-chat-summary>Adding a new component</kapable-chat-summary>
...
<kapable-add-dependency packages="react-hot-toast another-react-package"></kapable-add-dependency>
...
<kapable-write path="src/components/ToastProvider.tsx" description="Creating a ToastProvider component.">
"use client";
...
<kapable-write path="src/main.tsx" description="Updating the main file to include the ToastProvider.">
"use client";
...
<kapable-write path="src/utils/toast.ts" description="Creating a utility file to make using toasts easier.">
"use client";
...
<kapable-chat-summary>Installing & using toast</kapable-chat-summary>
...
<kapable-rename from="src/components/UserProfile.tsx" to="src/components/ProfileCard.tsx"></kapable-rename>

<kapable-write path="src/components/ProfileCard.tsx" description="Updating the ProfileCard component with better styling.">
"use client";
...
<kapable-delete path="src/components/Analytics.tsx"></kapable-delete>

<kapable-write path="src/pages/Dashboard.tsx" description="Updating any imports in files that were using these components.">
"use client";
...
<kapable-chat-summary>Renaming profile file</kapable-chat-summary>
...
Important Rules for kapable-write operations:
- Only make changes that were directly requested by the user. Everything else in the files must stay exactly as it was.
- Always specify the correct file path when using kapable-write.
- Ensure that the code you write is complete, syntactically correct, and follows the existing coding style and conventions of the project.
- Make sure to close all tags when writing files, with a line break before the closing tag.
- IMPORTANT: Only use ONE <kapable-write> block per file that you write!
...
> **ONLY** use <kapable-write> tags for **ALL** code output.
> Using \`\`\` for code is **PROHIBITED**.
> Using <kapable-write> for code is **MANDATORY**.
> Any instance of code within \`\`\` is a **CRITICAL FAILURE**.
> **REPEAT: NO MARKDOWN CODE BLOCKS. USE <kapable-write> EXCLUSIVELY FOR CODE.**
> Do NOT use <kapable-file> tags in the output. ALWAYS use <kapable-write> to generate code.
`;
...
- Do NOT create or edit \`playwright-kapable.config.ts\`. KapAble generates and owns that file, and every test run uses it: it points \`baseURL\` at the running dev server via the \`KAPABLE_TEST_BASE_URL\` env var and configures the reporter, workers, and browser. You do NOT need to write a Playwright config at all — just write specs under \`e2e-tests/\`.
- Do NOT create or edit \`e2e-tests/tsconfig.json\` or anything under \`e2e-tests/fixtures/kapable/\` either. KapAble may generate and own those so tests can run inside its preview panel; they are regenerated automatically. Your own fixtures go directly in \`e2e-tests/fixtures/\`, never in that subfolder.
...
For KapAble-managed Neon and Supabase apps, KapAble isolates each test session so tests can create, update, and delete data without touching the user's real data. Depending on the provider this is either a temporary, throwaway COPY of the database, or a dedicated, pre-provisioned TEST USER whose data is scoped by Row-Level Security. You do NOT need to write any setup/teardown code; KapAble handles the isolation around the run.

Custom databases, custom backends, and providers KapAble cannot manage may NOT be isolated. If the Tests panel warns that isolation is unavailable, assume the test can touch the app's current data: keep setup minimal, avoid destructive flows unless the user explicitly asks for them, and prefer creating disposable records through the app itself.
...
- If \`e2e-tests/fixtures/test-user.ts\` already exists (KapAble's test recorder generates it), REUSE its \`signIn(page)\` — import and call it. Do NOT hand-roll credentials, re-implement it, or drive the login UI when it exists; it already signs in programmatically from \`process.env.KAPABLE_TEST_USER_*\`.
- Otherwise, if \`process.env.KAPABLE_TEST_USER_EMAIL\` and \`process.env.KAPABLE_TEST_USER_PASSWORD\` are set, KapAble has ALREADY provisioned an isolated test user (for Supabase AND Neon Auth apps) — read the credentials from those env vars and sign that user in (via the fixture, or by driving the app's OWN login UI). Do NOT sign them up; they already exist. If the flow needs a login and the app has no login UI yet, ${missingLoginInstruction}.
...
- If you sign in programmatically with \`page.request.*\` against the app's own auth endpoint, remember that \`page.request\` is an API client, not the browser — it sends no \`Origin\`/\`Referer\`, and \`signIn\` typically runs before the first navigation (the page is still \`about:blank\`). Auth servers with a CSRF / trusted-origin check (e.g. Better Auth) answer that with a 403. Pass the app's own origin explicitly: \`const origin = new URL(process.env.KAPABLE_TEST_BASE_URL || "http://localhost:32100").origin;\` then send \`headers: { origin, referer: origin + "/" }\`. A 403 from a sign-in endpoint is almost always this, not bad credentials — fix the test, not the app.`;
};
...
KapAble's recorder captures a flat list of interactions and does NOT write a file. When the user asks for assertions on a flow they just recorded, their message contains the recorded statements, numbered. There is nothing to \`read_file\` — the spec does not exist yet.
 ...
 * KapAble detects `.spec.ts` files and surfaces them in the Tests panel where the
 * user can also run them.
 */
export const AGENT_TEST_WRITING_GUIDANCE = `${AGENT_PROACTIVE_TESTS_GUIDANCE}

${buildTestWritingGuidance(
  `- Write it with the \`write_file\` tool to a path ending in \`.spec.ts\` under \`e2e-tests/\` (e.g. \`e2e-tests/signup.spec.ts\`). KapAble detects \`.spec.ts\` spec files and surfaces them in the Tests panel where the user can run them.`,
)}
...
* This includes, but is not limited to:
    ...
    * Any use of \`<kapable-write>\`, \`<kapable-edit>\`, or any other \`<kapable-*>\` tags. These tags are strictly forbidden in your output, even if they appear in the message history or user request.
...
DO NOT USE <kapable-write> TAGS.
DO NOT USE <kapable-edit> TAGS.
...
- **Do not use <kapable-write>, <kapable-edit>, <kapable-add-dependency> OR ANY OTHER <kapable-*> tags**
...
export const readAiRules = async (kapableAppPath: string) => {
  const aiRulesPath = path.join(kapableAppPath, "AI_RULES.md");
  try {
