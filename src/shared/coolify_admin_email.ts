...
const UNSENDABLE =
  "KapAble can't send an address containing quotes, backslashes or ` $ # ! to " +
 ...
 * rules below is KapAble's own limit, and an address it refuses may be one
 ...
 * One is about where mail goes; the other is about what KapAble can put in a
...
export function adminEmailRefusal(email: string): string | null {
  ...
  // rather than after KapAble has connected and looked the server over.
  //
  // Some of these are legal in a local part, so this is KapAble's limit rather
  ...
  // below rather than letting `kapable.test.` past the reserved list.
