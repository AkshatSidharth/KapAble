...
`setupChatFlowHarness` runs the **real** kapable chat flow as a fast vitest
...
counters), and the real response processor (kapable-tag parsing, file writes, git
...
| `useFakeCatalog` | `true`                                                                             | Point `KAPABLE_LANGUAGE_MODEL_CATALOG_URL` at the fake server.        |
| `engine`         | `false`                                                                            | Point `KAPABLE_ENGINE_URL` / `KAPABLE_GATEWAY_URL` at the fake server.   |
...
process.env.KAPABLE_E2E_PORT_BLOCK_INDEX = "4"; // app 40300+, proxy 41300+
```
...
- `tc=kapable-write-angle` → streams a `<kapable-write path="src/foo/bar.tsx">` (see
  `kapable_tags_parsing.integration.test.ts`).
...
(a `<kapable-write path="file1.txt">`), **not** an echo — mirror what the fixture
...
```ts
// Files written by kapable-write tags:
...
`[[kapable-dump-path=...]]` in its reply. `harness.getServerDump()` reads the
...
- `.gitattributes` kapable-file blocks and `package.json` kapable-file blocks stripped.
- Tool-call ids → `[[TOOL_CALL_n]]`; MCP `call-id="..."` → `[[MCP_CALL_ID_n]]`.
- `[[kapable-dump-path=...]]` → `[[kapable-dump-path=*]]`; compaction backup paths →
...
- **`getServerDump` needs a dump to exist.** It throws if no `[[kapable-dump-path]]`
...
- **KapAble Pro / engine routing**: pass `engine: true` to point
  `KAPABLE_ENGINE_URL` / `KAPABLE_GATEWAY_URL` at the harness fake server. Model-client
...
- Harness-created chats have `chatMode: null`; with KapAble Pro enabled,
