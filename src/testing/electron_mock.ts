 ...
 *     harness via the `KAPABLE_DEV_USER_DATA_DIR` env var (read at call time so the
...
function resolveUserDataPath(): string {
  return (
    process.env.KAPABLE_DEV_USER_DATA_DIR ||
    process.env.KAPABLE_TEST_USER_DATA_DIR ||
    `${process.env.TMPDIR || "/tmp"}/kapable-vitest-userdata-${process.pid}`
  );
}
    ...
    utilityProcess: {
      ...
      fork: vi.fn((modulePath: string, _args?: string[], _opts?: unknown) => {
        ...
        const child = fork(
          ...
          [],
          {
            env: { ...process.env, KAPABLE_TEST_WORKER_BUNDLE: bundle },
