 ...
 * file and embeds `[[kapable-dump-path=<file>]]` in its streamed reply. This
  ...
  if (Array.isArray(content)) {
    return content.map((part) => {
      if (
        ...
        return {
          ...part,
          text: removeIgnoredKapableFileBlocks((part as { text: string }).text),
        };
      }
      return part;
    });
...
/** Extract every `[[kapable-dump-path=...]]` path from message text, in order. */
export function extractDumpPaths(text: string): string[] {
  const matches = text.match(/\[\[kapable-dump-path=([^\]]+)\]\]/g) ?? [];
  return matches
    .map((m) => m.match(/\[\[kapable-dump-path=([^\]]+)\]\]/)?.[1])
