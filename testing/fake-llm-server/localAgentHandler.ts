...
export function convertLegacyFixtureToLocalAgent(
  ...
  const turns: Turn[] = [];
  const tagPattern =
    /<kapable-(write|delete|rename|add-dependency|execute-sql|search-replace|add-integration|chat-summary)\b([^>]*)>([\s\S]*?)<\/kapable-\1>/g;
  let precedingEnd = 0;
 ...
 * The user message format includes: "path: /path/to/app/.kapable/media/hash.png"
 */
