...
const VERIFY_REQUEST_RE =
  /^I approved the assertions\. KapAble generated (\S+) from my recording\.\s*$/m;
...
const APPROVED_RESULT_RE =
  /^The user approved the plan\. KapAble generated (\S+) from the recording/m;
...
export function matchAssertionsAgentTurn(
  ...
  // out its timeout on a missing `kapable-test-assertions-card` with nothing
  ...
  return {
    ...
    args: {
      ...
      recordingId,
      // The model names the test. KapAble only uses this when the user left the
