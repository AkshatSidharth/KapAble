 ...
 * session starts, the KapAble renderer sends the isolated test user's credentials
 ...
 *   down (from parent): { type: "kapable-auth-login", auth, nonce, token }
 *   up   (to parent):   { type: "kapable-auth-bootstrap-ready", pendingNonce? }
 *                       { type: "kapable-auth-ready", ok: boolean, error?: string,
 *                         path?: string }
 *
 ...
 * lives in sessionStorage — scoped to the long-lived KapAble window, not to the
 * attempt. The nonce is how the new document tells "the marker this attempt just
 * wrote" from "one an earlier attempt abandoned".
 */
(() => {
  // A source check proves only which window sent the message, not that KapAble is
  ...
  const authBootstrapToken = document.currentScript?.dataset.kapableAuthToken;
  try {
    document.currentScript?.removeAttribute("data-kapable-auth-token");
  } catch {
    // Best effort: the value is already snapshotted above either way.
  }
  const PENDING_KEY = "__kapable_auth_pending__";
  ...
  function post(ok, error, path) {
    window.parent.postMessage(
      { type: "kapable-auth-ready", ok, error, path, nonce: activeNonce },
      "*",
    );
  }
  ...
  function checkPendingOnLoad() {
    ...
    // parent resends `kapable-auth-login` in response.
    window.parent.postMessage(
      {
        type: "kapable-auth-bootstrap-ready",
