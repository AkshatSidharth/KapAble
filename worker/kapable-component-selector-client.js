(() => {
  const OVERLAY_CLASS = "__kapable_overlay__";
  let overlays = [];
    ...
    if (highlightedElement) {
      ...
      if (highlightedItem) {
        ...
        window.parent.postMessage(
          {
            type: "kapable-component-coordinates-updated",
            coordinates: {
              top: rect.top,
              left: rect.left,
              width: rect.width,
   ...
   * Returns the parent kapable-tagged element if the current one is an overlay,
  ...
  function skipOverlayElement(el) {
    ...
    // This looks like an overlay — walk up to the parent with a kapable-id
  ...
  function onClick(e) {
    ...
    if (selectedItem && (highlightedElement === state.element || !isProMode)) {
      ...
      window.parent.postMessage(
        {
          type: "kapable-component-deselected",
    ...
    if (!state.element.dataset.kapableRuntimeId) {
      state.element.dataset.kapableRuntimeId = `kapable-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
    }

    const rect = state.element.getBoundingClientRect();
    window.parent.postMessage(
      {
        type: "kapable-component-selected",
        component: {
          id: clickedComponentId,
          name: state.element.dataset.dyadName,
          runtimeId: state.element.dataset.kapableRuntimeId,
        },
        coordinates: {
          top: rect.top,
          left: rect.left,
  ...
  function onKeyDown(e) {
    ...
    // Electron's native reload accelerators target the whole KapAble renderer.
    ...
    if (key === "r" && hasCtrlOrMeta) {
      ...
      window.parent.postMessage(
        {
          type: "kapable-preview-reload-shortcut",
        },
        "*",
      );
      return;
    ...
    if (key === "c" && hasShift && hasCtrlOrMeta) {
      ...
      window.parent.postMessage(
        {
          type: "kapable-select-component-shortcut",
        },
        "*",
      );
    }
  ...
  window.addEventListener("message", (e) => {
    ...
    // can deliver it to KapAble with the source identity the parent expects.
    if (
      e.source !== window.parent &&
      e.origin === window.location.origin &&
      e.data?.type === "kapable-preview-reload-shortcut"
    ) {
      window.parent.postMessage(e.data, "*");
      return;
    }
    if (e.source !== window.parent) return;
    if (e.data.type === "kapable-pro-mode") {
      isProMode = e.data.enabled;
    }
    if (e.data.type === "activate-kapable-component-selector") activate();
    if (e.data.type === "deactivate-kapable-component-selector") deactivate();
    if (e.data.type === "activate-kapable-visual-editing") {
      activate();
    }
    if (e.data.type === "deactivate-kapable-visual-editing") {
      deactivate();
      clearOverlays();
    }
    if (e.data.type === "clear-kapable-component-overlays") clearOverlays();
    if (e.data.type === "update-kapable-overlay-positions") {
    ...
    if (
      e.data.type === "remove-kapable-component-overlay" ||
      e.data.type === "deselect-kapable-component"
    ) {
      if (e.data.componentId) {
        removeOverlayById(e.data.componentId);
      }
    }
    if (e.data.type === "restore-kapable-component-overlays") {
  ...
  function initializeComponentSelector() {
    if (!document.body) {
      console.error(
        "KapAble component selector initialization failed: document.body not found.",
      );
      return;
    }
    ...
    function checkForTaggedElements() {
      if (document.body.querySelector("[data-dyad-id]")) {
        ...
        window.parent.postMessage(
          {
            type: "kapable-component-selector-initialized",
          },
          "*",
        );
        console.debug("KapAble component selector initialized");
        return true;
      }
      return false;
    }
    ...
    setTimeout(() => {
      ...
      console.debug(
        "KapAble component selector waiting for tagged elements to appear...",
      );
      ...
      timeoutId = setTimeout(() => {
        ...
        if (!document.body.querySelector("[data-dyad-id]")) {
          console.warn(
            "KapAble component selector not initialized because no DOM elements were tagged",
          );
        }
      }, INIT_TIMEOUT_MS);
    }, 0);
