 ...
 *
 * Protocol:
 *   down (from parent): { type: "activate-kapable-recorder" | "deactivate-kapable-recorder", token }
 *                       { type: "flush-kapable-recorder", token, requestId }
 *   up   (to parent):   { type: "kapable-recorder-initialized" }
 *                       { type: "kapable-recorder-action", action: RecordedAction }
 *                       { type: "kapable-recorder-flushed", requestId }
 *
 * Known limitation — the top-level preview document only. This script is
 * injected into nested frames too, but KapAble activates only the frame it embeds,
...
(() => {
  ...
  const allowUntrustedEvents =
    window.__KAPABLE_RECORDER_ALLOW_UNTRUSTED__ === true;
  ...
  const recorderToken = document.currentScript?.dataset.kapableRecorderToken;
  try {
    document.currentScript?.removeAttribute("data-kapable-recorder-token");
  } catch {
    // Best effort: the value is already snapshotted above either way.
  }
  try {
    delete window.__KAPABLE_RECORDER_ALLOW_UNTRUSTED__;
  } catch {
  ...
  const OVERLAY_CLASS = "__kapable_recorder_overlay__";
  ...
  function buildCandidates(el) {
    ...
    // KapAble plugin that injects it doesn't run in the build the test replays
  ...
  /**
   * Find the source location KapAble's development tagger attached to this DOM
  ...
  function emit(action) {
    ...
    lastEmit = { key, at: now };
    window.parent.postMessage({ type: "kapable-recorder-action", action }, "*");
  }
  ...
  // whoever asked. This script only has a job inside KapAble's preview frame.
  ...
  window.addEventListener("message", (e) => {
    ...
    if (type === "activate-kapable-recorder") activate();
    else if (type === "deactivate-kapable-recorder") deactivate();
    else if (
      type === "flush-kapable-recorder" &&
      typeof e.data.requestId === "string"
    ) {
      window.parent.postMessage(
        { type: "kapable-recorder-flushed", requestId: e.data.requestId },
        "*",
      );
    }
  });
  ...
  function init() {
    window.parent.postMessage({ type: "kapable-recorder-initialized" }, "*");
    console.debug("KapAble recorder client initialized");
  }
