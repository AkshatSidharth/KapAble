(() => {
  ...
  function getScreenshotDimensions() {
    ...
    console.warn(
      `[kapable-screenshot] Full page ${fullPage.width}x${fullPage.height} exceeds the screenshot memory budget; capturing viewport ${boundedViewport.width}x${boundedViewport.height} instead.`,
    );
    return {
      ...boundedViewport,
      fullPage,
  ...
  async function handleScreenshotRequest(requestId) {
    try {
      console.debug("[kapable-screenshot] Capturing screenshot...");

      const dataUrl = await captureScreenshot();

      console.debug("[kapable-screenshot] Screenshot captured successfully");
      ...
      console.error("[kapable-screenshot] Screenshot capture failed:", error);
  ...
  window.addEventListener("message", (event) => {
    ...
    if (event.data.type === "kapable-take-screenshot") {
