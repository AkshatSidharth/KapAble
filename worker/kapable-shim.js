(function () {
  console.debug("kapable-shim.js loaded via proxy v0.6.0");
   ...
   * one case that matters, KapAble's own `location.replace` navigation.
   */
      ...
      if (url && typeof url === "string") {
        try {
          ...
          if (
            ...
            console.warn(
              "[kapable-shim] Blocked navigation to unsafe URL protocol:",
              parsedUrl.protocol,
            );
          }
        } catch (e) {
          console.error("[kapable-shim] Invalid navigation URL:", e);
        }
      } else if (url) {
        console.warn("[kapable-shim] Invalid URL type:", typeof url);
      } else {
