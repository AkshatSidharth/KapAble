(() => {
  ...
  function findElementByKapableId(kapableId, runtimeId) {
    ...
    // Fall back to finding by kapable-id (will get first match)
    const escaped = CSS.escape(kapableId);
  ...
  function applyStyles(element, styles) {
    ...
    console.debug(
      `[KapAble Visual Editor] Applying styles:`,
      styles,
      "to element:",
      element,
    );
  ...
  function handleGetStyles(data) {
    ...
    const element = findElementByKapableId(elementId, runtimeId);
    if (element) {
  ...
  function handleEnableTextEditing(data) {
    ...
    const element = findElementByKapableId(componentId, runtimeId);
    if (element) {
  ...
  function handleGetTextContent(data) {
    ...
    window.parent.postMessage(
      {
        type: "kapable-text-content-response",
        componentId,
  ...
  function handleModifyImageSrc(data) {
    ...
    if (imgEl) {
      ...
      if (imgEl._kapableAbort) imgEl._kapableAbort.abort();
      const controller = new AbortController();
      imgEl._kapableAbort = controller;
  ...
  window.addEventListener("message", (e) => {
    ...
    switch (type) {
      case "get-kapable-component-styles":
        handleGetStyles(data);
        break;
      case "modify-kapable-component-styles":
        handleModifyStyles(data);
        break;
      case "enable-kapable-text-editing":
        handleEnableTextEditing(data);
        break;
      case "disable-kapable-text-editing":
        handleDisableTextEditing(data);
        break;
      case "get-kapable-text-content":
        handleGetTextContent(data);
        break;
      case "modify-kapable-image-src":
